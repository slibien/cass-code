...
Cass configuration gives you fine-grained control over the model, execution environment, and integrations available to the CLI. Use this guide alongside the workflows in [`cass exec`](./exec.md), the guardrails in [Sandbox & approvals](./sandbox.md), and project guidance from [AGENTS.md discovery](./agents_md.md).
...
Cass supports several mechanisms for setting config values:
...
- A generic `-c`/`--config` flag that takes a `key=value` pair, such as `--config model="o3"`.
  ...
  - For consistency with `config.toml`, values are a string in TOML format rather than JSON format, so use `key='{a = 1, b = 2}'` rather than `key='{"a": 1, "b": 2}'`.
    - The quotes around the value are necessary, as without them your shell would split the config argument on spaces, resulting in `cass` receiving `-c key={a` with (invalid) additional arguments `=`, `1,`, `b`, `=`, `2}`.
...
- The `$CODEX_HOME/config.toml` configuration file where the `CODEX_HOME` environment value defaults to `~/.cass`. (Note `CODEX_HOME` will also be where logs and other Cass-related information are stored.)
...
The model that Cass should use.

```toml
model = "gpt-5"  # overrides the default ("gpt-5-cass" on macOS/Linux, "gpt-5" on Windows)
```

### model_providers

This option lets you add to the default set of model providers bundled with Cass. The map key becomes the value you use with `model_provider` to select the provider.
...
# Name of the provider that will be displayed in the Cass UI.
name = "OpenAI using Chat Completions"
# The path `/chat/completions` will be amended to this URL to make the POST
# request for the chat completions.
base_url = "https://api.openai.com/v1"
# If `env_key` is set, identifies an environment variable that must be set when
# using Cass with this provider. The value of the environment variable must be
...
Note this makes it possible to use Cass CLI with non-OpenAI models, so long as they use a wire API that is compatible with the OpenAI chat completions API. For example, you could define the following provider to use Cass CLI with Ollama running locally:
...
Export your key before launching Cass: `export AZURE_OPENAI_API_KEY=…`
...
How many times Cass will retry a failed HTTP request to the model provider. Defaults to `4`.

##### stream_max_retries

Number of times Cass will attempt to reconnect when a streaming response is interrupted. Defaults to `5`.

##### stream_idle_timeout_ms

How long Cass will wait for activity on a streaming response before treating the connection as lost. Defaults to `300_000` (5 minutes).
...
If the selected model is known to support reasoning (for example: `o3`, `o4-mini`, `cass-*`, `gpt-5`, `gpt-5-cass`), reasoning is enabled by default when using the Responses API. As explained in the [OpenAI Platform documentation](https://platform.openai.com/docs/guides/reasoning?api-mode=responses#get-started-with-reasoning), this can be set to:
...
If the model name starts with `"o"` (as in `"o3"` or `"o4-mini"`) or `"cass"`, reasoning is enabled by default when using the Responses API. As explained in the [OpenAI Platform documentation](https://platform.openai.com/docs/guides/reasoning?api-mode=responses#reasoning-summaries), this can be set to:
...
When set, Cass includes a `text` object in the request payload with the configured verbosity, for example: `"text": { "verbosity": "low" }`.
...
In general, Cass knows the context window for the most common OpenAI models, but if you are using a new model with an old version of the Cass CLI, then you can use `model_context_window` to tell Cass what value to use to determine how much context is left during a conversation.

### model_max_output_tokens

This is analogous to `model_context_window`, but for the maximum number of output tokens for the model.

> See also [`cass exec`](./exec.md) to see how these model settings influence non-interactive runs.

## Execution environment

### approval_policy

Determines when the user should be prompted to approve whether Cass can execute a command:

```toml
# Cass has hardcoded logic that defines a set of "trusted" commands.
# Setting the approval_policy to `untrusted` means that Cass will prompt the
# user before running a command not in the "trusted" set.
#
# See https://github.com/slibien/cass-code/issues/1260 for the plan to enable
...
```toml
# If the command fails when run in the sandbox, Cass asks for permission to
...
```toml
# User is never prompted: if the command fails, Cass will automatically try
...
Cass executes model-generated shell commands inside an OS-level sandbox.
...
A more relaxed policy is `workspace-write`. When specified, the current working directory for the Cass task will be writable (as well as `$TMPDIR` on macOS). Note that the CLI defaults to using the directory where it was spawned as `cwd`, though this can be overridden using `--cwd/-C`.

On macOS (and soon Linux), all writable roots (including `cwd`) that contain a `.git/` folder _as an immediate child_ will configure the `.git/` folder to be read-only while the rest of the Git repository will be writable. This means that commands like `git commit` will fail, by default (as it entails writing to `.git/`), and will require Cass to ask for permission.
...
# By default, the cwd for the Cass session will be writable as well as $TMPDIR
...
This is reasonable to use if Cass is running in an environment that provides its own sandboxing (such as a Docker container) such that further sandboxing is unnecessary.

Though using this option may also be necessary if you try to use Cass in environments where its native sandboxing mechanisms are unsupported, such as older Linux kernels or on Windows.
...
```toml
[tools]
web_search = true   # allow Cass to issue first-party web searches without prompting you
view_image = false  # disable image uploads (they're enabled by default)
```

`web_search` is also recognized under the legacy name `web_search_request`. The `view_image` toggle is useful when you want to include screenshots or diagrams from your repo without pasting them manually. Cass still respects sandboxing: it can only attach files inside the workspace roots you allow.

### approval_presets

Cass provides three main Approval Presets:

- Read Only: Cass can read files and answer questions; edits, running commands, and network access require approval.
- Auto: Cass can read files, make edits, and run commands in the workspace without approval; asks for approval outside the workspace or for network access.
- Full Access: Full disk and network access without prompts; extremely risky.

You can further customize how Cass runs at the command line using the `--ask-for-approval` and `--sandbox` options.

> See also [Sandbox & approvals](./sandbox.md) for in-depth examples and platform-specific behaviour.

### shell_environment_policy

Cass spawns subprocesses (e.g. when executing a `local_shell` tool-call suggested by the assistant). By default it now passes **your full environment** to those subprocesses. You can tune this behavior via the **`shell_environment_policy`** block in `config.toml`:
...
| `ignore_default_excludes` | boolean              | `false` | When `false`, Cass removes any var whose **name** contains `KEY`, `SECRET`, or `TOKEN` (case-insensitive) before other rules run.              |
...
You can configure Cass to use [MCP servers](https://modelcontextprotocol.io/about) to give Cass access to external applications, resources, or services.
...
# https://github.com/slibien/cass-code/blob/main/cass-rs/rmcp-client/src/utils.rs#L82
env = { "API_KEY" = "value" }
# or
...
[Streamable HTTP servers](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports#streamable-http) enable Cass to talk to resources that are accessed via a http url (either on localhost or another domain).
...
After enabling it, run `cass mcp login <server-name>` when the server supports OAuth.
...
When both `enabled_tools` and `disabled_tools` are specified, Cass first restricts the server to the allow-list and then removes any tools that appear in the deny-list.
...
cass mcp add docs -- docs-server --port 4000
...
cass mcp get docs
cass mcp get docs --json

# Remove a server
cass mcp remove docs

# Log in to a streamable HTTP server that supports oauth
cass mcp login SERVER_NAME

# Log out from a streamable HTTP server that supports oauth
cass mcp logout SERVER_NAME
```

### Examples of useful MCPs

There is an ever growing list of useful MCP servers that can be helpful while you are working with Cass.
...
- [Sentry](https://docs.sentry.io/product/sentry-mcp/#cass) — access to your Sentry logs
...
Cass can emit [OpenTelemetry](https://opentelemetry.io/) **log events** that
...
Cass tags every exported event with `service.name = $ORIGINATOR` (the same
value sent in the `originator` header, `cass_cli_rs` by default), the CLI
version, and an `env` attribute so downstream collectors can distinguish
dev/staging/prod traffic. Only telemetry produced inside the `cass_otel`
...
With OTEL enabled Cass emits the following event types (in addition to the
metadata above):

- `cass.conversation_starts`
  - `provider_name`
...
If you build Cass from source the OTEL crate is still behind an `otel` feature
...
Specify a program that will be executed to get notified about events generated by Cass. Note that the program will receive the notification argument as a string of JSON, e.g.:
...
`"thread-id"` contains a string that identifies the Cass session that produced the notification; you can use it to correlate multiple turns that belong to the same task.
    ...
    match notification_type := notification.get("type"):
        case "agent-turn-complete":
            ...
            if assistant_message:
                title = f"Cass: {assistant_message}"
            else:
                title = "Cass: Turn Complete!"
...
To have Cass use this script for notifications, you would configure it via `notify` in `~/.cass/config.toml` using the appropriate path to `notify.py` on your computer:

```toml
notify = ["python3", "/Users/mbolin/.cass/notify.py"]
```

> [!NOTE]
> Use `notify` for automation and integrations: Cass invokes your external program with a single JSON argument for each event, independent of the TUI. If you only want lightweight desktop notifications while using the TUI, prefer `tui.notifications`, which uses terminal escape codes and requires no external program. You can enable both; `tui.notifications` covers in‑TUI alerts (e.g., approval prompts), while `notify` is best for system‑level hooks or custom notifiers. Currently, `notify` emits only `agent-turn-complete`, whereas `tui.notifications` supports `agent-turn-complete` and `approval-requested` with optional filtering.

### hide_agent_reasoning

Cass intermittently emits "reasoning" events that show the model's internal "thinking" before it produces a final answer. Some users may find these events distracting, especially in CI logs or minimal terminal output.
...
4. the default value that comes with Cass CLI (i.e., Cass CLI defaults to `gpt-5-cass`)

### history

By default, Cass CLI records messages sent to the model in `$CODEX_HOME/history.jsonl`. Note that on UNIX, the file permissions are set to `o600`, so it should only be readable and writable by the owner.
...
Currently, `"vscode"` is the default, though Cass does not verify VS Code is installed. As such, `file_opener` may default to `"none"` or something else in the future.
...
> See also [AGENTS.md discovery](./agents_md.md) for how Cass locates these files during a session.
...
> [!NOTE]
> Cass emits desktop notifications using terminal escape codes. Not all terminals support these (notably, macOS Terminal.app and VS Code's terminal do not support custom notifications. iTerm2, Ghostty and WezTerm do support these notifications).
...
To force users on a given machine to use a specific login method or workspace, use a combination of [managed configurations](https://developers.orion-ai.dev/cass-code/security#managed-configuration) as well as either or both of the following fields:
...
# flow and the id will be validated during the oauth callback as well as every time Cass starts.
forced_chatgpt_workspace_id = "00000000-0000-0000-0000-000000000000"
```

If the active credentials don't match the config, the user will be logged out and Cass will exit.
...
| ------------------------------------------------ | ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `model`                                          | string                                                            | Model to use (e.g., `gpt-5-cass`).                                                                                        |
...
| `mcp_servers.<id>.enabled`                       | boolean                                                           | When false, Cass skips starting the server (default: true).                                                               |
...
| `tools.view_image`                               | boolean                                                           | Enable or disable the `view_image` tool so Cass can attach local image files from the workspace (default: true).          |
| `forced_login_method`                            | `chatgpt` \| `api`                                                | Only allow Cass to be used with ChatGPT or API keys.                                                                      |
| `forced_chatgpt_workspace_id`                    | string (uuid)                                                     | Only allow Cass to be used with the specified ChatGPT workspace.                                                          |

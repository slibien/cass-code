...
impl McpProcess {
    ...
    pub async fn new_with_env(
        ...
        let std_cmd = StdCommand::cargo_bin("cass-mcp-server")
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let user_agent = format!(
            "cass_cli_rs/0.0.0 ({} {}; {}) {} (elicitation test; 0.0.0)",
            os_info.os_type(),
            os_info.version(),
            os_info.architecture().unwrap_or("unknown"),
            cass_core::terminal::user_agent()
        );
        assert_eq!(
    ...
    pub async fn send_cass_tool_call(
        &mut self,
        params: CassToolCallParam,
    ) -> anyhow::Result<i64> {
        let cass_tool_call_params = CallToolRequestParams {
            name: "cass".to_string(),
            arguments: Some(serde_json::to_value(params)?),
        };
        self.send_request(
            mcp_types::CallToolRequest::METHOD,
            Some(serde_json::to_value(cass_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "cass/event" with params.msg.type == "task_complete".
        ...
        loop {
            ...
            match message {
                JSONRPCMessage::Notification(notification) => {
                    let is_match = if notification.method == "cass/event" {

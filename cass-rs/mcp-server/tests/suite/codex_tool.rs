...
use cass_mcp_server::CassToolCallParam;
use cass_mcp_server::ExecApprovalElicitRequestParams;
use cass_mcp_server::ExecApprovalResponse;
use cass_mcp_server::PatchApprovalElicitRequestParams;
use cass_mcp_server::PatchApprovalResponse;
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "cass" tool request, which should hit the completions endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let cass_request_id = mcp_process
        .send_cass_tool_call(CassToolCallParam {
    ...
    let expected_elicitation_request = create_expected_elicitation_request(
        ...
        cass_request_id.to_string(),
        params.cass_event_id.clone(),
    )?;
    ...
    let cass_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Integer(cass_request_id)),
    )
    .await??;
    assert_eq!(
        JSONRPCResponse {
            jsonrpc: JSONRPC_VERSION.into(),
            id: RequestId::Integer(cass_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request(
    ...
    command: Vec<String>,
    workdir: &Path,
    cass_mcp_tool_call_id: String,
    cass_event_id: String,
) -> anyhow::Result<JSONRPCRequest> {
    let expected_message = format!(
        "Allow Cass to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let cass_parsed_cmd = parse_command::parse_command(&command);
    Ok(JSONRPCRequest {
        jsonrpc: JSONRPC_VERSION.into(),
        ...
        params: Some(serde_json::to_value(&ExecApprovalElicitRequestParams {
            ...
            cass_elicitation: "exec-approval".to_string(),
            cass_mcp_tool_call_id,
            cass_event_id,
            cass_command: command,
            cass_cwd: workdir.to_path_buf(),
            cass_call_id: "call1234".to_string(),
            cass_parsed_cmd,
            cass_risk: None,
        })?),
    })
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "cass" tool request that will trigger the apply_patch command
    let cass_request_id = mcp_process
        .send_cass_tool_call(CassToolCallParam {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_cass_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = cass_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn cass_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    create_config_toml(cass_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(cass_home.path()).await?;
...
fn create_expected_patch_approval_elicitation_request(
    ...
    message_lines.push("Allow Cass to apply proposed code changes?".to_string());
    ...
    Ok(JSONRPCRequest {
        ...
        params: Some(serde_json::to_value(&PatchApprovalElicitRequestParams {
            ...
            cass_elicitation: "patch-approval".to_string(),
            cass_mcp_tool_call_id,
            cass_event_id,
            cass_reason: reason,
            cass_grant_root: grant_root,
            cass_changes: changes,
            cass_call_id: "call1234".to_string(),
        })?),
    })
}
...
/// Create a Cass config that uses the mock server as the model provider.

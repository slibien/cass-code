...
use crate::cass_tool_config::CassToolCallParam;
use crate::cass_tool_config::CassToolCallReplyParam;
use crate::cass_tool_config::create_tool_for_cass_tool_call_param;
use crate::cass_tool_config::create_tool_for_cass_tool_call_reply_param;
...
use cass_core::protocol::Submission;
...
pub(crate) struct MessageProcessor {
    ...
    running_requests_id_to_cass_uuid: Arc<Mutex<HashMap<RequestId, ConversationId>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            running_requests_id_to_cass_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_initialize(
        ...
        let result = mcp_types::InitializeResult {
            ...
            server_info: mcp_types::Implementation {
                name: "cass-mcp-server".to_string(),
                version: env!("CARGO_PKG_VERSION").to_string(),
                title: Some("Cass".to_string()),
                user_agent: Some(get_cass_user_agent()),
            },
        };
    ...
    async fn handle_list_tools(
        ...
        let result = ListToolsResult {
            tools: vec![
                create_tool_for_cass_tool_call_param(),
                create_tool_for_cass_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(
        ...
        match name.as_str() {
            "cass" => self.handle_tool_call_codex(id, arguments).await,
            "cass-reply" => {
                self.handle_tool_call_cass_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_codex(&self, id: RequestId, arguments: Option<serde_json::Value>) {
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<CassToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg
                    .into_config(self.cass_linux_sandbox_exe.clone())
                    .await
                {
                    Ok(cfg) => cfg,
                    Err(e) => {
                        let result = CallToolResult {
                            content: vec![ContentBlock::TextContent(TextContent {
                                ...
                                text: format!(
                                    "Failed to load Cass configuration from overrides: {e}"
                                ),
                                annotations: None,
                            })],
                            is_error: Some(true),
                ...
                Err(e) => {
                    let result = CallToolResult {
                        content: vec![ContentBlock::TextContent(TextContent {
                            r#type: "text".to_owned(),
                            text: format!("Failed to parse configuration for Cass tool: {e}"),
                            annotations: None,
                        })],
                        is_error: Some(true),
                        structured_content: None,
        ...
        let running_requests_id_to_cass_uuid = self.running_requests_id_to_cass_uuid.clone();

        // Spawn an async task to handle the Cass session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Cass session and stream events back to the client.
            crate::cass_tool_runner::run_cass_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
                conversation_manager,
                running_requests_id_to_cass_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_cass_session_reply(
        &self,
        ...
        // parse arguments
        let CassToolCallReplyParam {
            conversation_id,
            prompt,
        } = match arguments {
            Some(json_val) => match serde_json::from_value::<CassToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
                    tracing::error!("Failed to parse Cass tool call reply parameters: {e}");
                    let result = CallToolResult {
                        content: vec![ContentBlock::TextContent(TextContent {
                            r#type: "text".to_owned(),
                            text: format!("Failed to parse configuration for Cass tool: {e}"),
        ...
        tokio::spawn({
            ...
            let running_requests_id_to_cass_uuid = running_requests_id_to_cass_uuid.clone();

            async move {
                crate::cass_tool_runner::run_cass_tool_session_reply(
                    codex,
                    outgoing,
                    request_id,
                    prompt,
    ...
    async fn handle_cancelled_notification(
        ...
        let conversation_id = {
            let map_guard = self.running_requests_id_to_cass_uuid.lock().await;
        ...
        // Obtain the Cass conversation from the server.
        let cass_arc = match self
            .conversation_manager
            .get_conversation(conversation_id)
            .await
        {
        ...
        // Submit interrupt to Cass.
        let err = cass_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: cass_core::protocol::Op::Interrupt,
            })
            .await;
        if let Err(e) = err {
            tracing::error!("Failed to submit interrupt to Cass: {e}");
            return;
        }
        // unregister the id so we don't keep it in the map
        self.running_requests_id_to_cass_uuid
            .lock()
            .await
            .remove(&request_id);
    }

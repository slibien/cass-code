...
use cass_core::protocol::SandboxCommandAssessment;
...
use crate::cass_tool_runner::INVALID_PARAMS_ERROR_CODE;
...
pub struct ExecApprovalElicitRequestParams {
    ...
    pub cass_elicitation: String,
    pub cass_mcp_tool_call_id: String,
    pub cass_event_id: String,
    pub cass_call_id: String,
    pub cass_command: Vec<String>,
    pub cass_cwd: PathBuf,
    pub cass_parsed_cmd: Vec<ParsedCommand>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub cass_risk: Option<SandboxCommandAssessment>,
}
...
pub(crate) async fn handle_exec_approval_request(
    ...
    event_id: String,
    call_id: String,
    cass_parsed_cmd: Vec<ParsedCommand>,
    cass_risk: Option<SandboxCommandAssessment>,
) {
    let escaped_command =
        shlex::try_join(command.iter().map(String::as_str)).unwrap_or_else(|_| command.join(" "));
    let message = format!(
        "Allow Cass to run `{escaped_command}` in `{cwd}`?",
    ...
    let params = ExecApprovalElicitRequestParams {
        ...
        cass_mcp_tool_call_id: tool_call_id.clone(),
        cass_event_id: event_id.clone(),
        cass_call_id: call_id,

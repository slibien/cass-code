...
use cass_core::features::Feature;
...
pub struct TestCassBuilder {
    config_mutators: Vec<Box<ConfigMutator>>,
}

impl TestCassBuilder {
    ...
    pub async fn build(&mut self, server: &wiremock::MockServer) -> anyhow::Result<TestCass> {
    ...
    async fn build_with_home(
        ...
        let conversation_manager = ConversationManager::with_auth(CassAuth::from_api_key("dummy"));

        let new_conversation = match resume_from {
            Some(path) => {
                let auth_manager = cass_core::AuthManager::from_auth_for_testing(
                    CassAuth::from_api_key("dummy"),
                );
    ...
    async fn prepare_config(
        ...
        if let Ok(cmd) = assert_cmd::Command::cargo_bin("cass") {
            config.cass_linux_sandbox_exe = Some(PathBuf::from(cmd.get_program().to_os_string()));
        }
...
pub struct TestCass {
    ...
    pub cwd: Arc<TempDir>,
    pub codex: Arc<CassConversation>,
...
impl TestCass {
    ...
    pub async fn submit_turn_with_policy(
        ...
        wait_for_event(&self.cass, |event| {
...
pub struct TestCassHarness {
    server: MockServer,
    test: TestCass,
}

impl TestCassHarness {
    ...
    pub async fn with_builder(mut builder: TestCassBuilder) -> Result<Self> {
...
pub fn test_codex() -> TestCassBuilder {
    TestCassBuilder {
        config_mutators: vec![],
    }
}

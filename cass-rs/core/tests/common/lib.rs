...
/// avoids clobbering a developer’s real `~/.cass`.
pub fn load_default_config_for_test(cass_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        cass_linux_sandbox_exe: Some(cargo_bin("cass-linux-sandbox")),
...
pub async fn wait_for_event<F>(
    codex: &CassConversation,
    predicate: F,
) -> cass_core::protocol::EventMsg
where
    F: FnMut(&cass_core::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(codex, predicate, Duration::from_secs(1)).await
}

pub async fn wait_for_event_match<T, F>(codex: &CassConversation, matcher: F) -> T
where
    F: Fn(&cass_core::protocol::EventMsg) -> Option<T>,
{
...
pub fn sandbox_env_var() -> &'static str {
    cass_core::spawn::CODEX_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    cass_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

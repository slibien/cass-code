...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestCass) -> (PathBuf, String) {
        match self {
...
impl Expectation {
    fn verify(&self, test: &TestCass, result: &CommandResult) -> Result<()> {
        match self {
...
async fn wait_for_completion_without_approval(test: &TestCass) {
    let event = wait_for_event_with_timeout(
        &test.cass,
        |event| {
            matches!(
...
async fn wait_for_completion(test: &TestCass) {
    wait_for_event(&test.cass, |event| {

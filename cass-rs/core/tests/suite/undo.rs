...
use core_test_support::test_codex::TestCassHarness;
use core_test_support::wait_for_event_match;
use pretty_assertions::assert_eq;

#[allow(clippy::expect_used)]
async fn undo_harness() -> Result<TestCassHarness> {
    TestCassHarness::with_config(|config: &mut Config| {
...
fn init_git_repo(path: &Path) -> Result<()> {
    ...
    git(path, &["config", "user.name", "Cass Tests"])?;
    git(path, &["config", "user.email", "cass-tests@example.com"])?;

    // Create README.txt
    let readme_path = path.join("README.txt");
    fs::write(&readme_path, "Test repository initialized by Cass.\n")?;
...
async fn invoke_undo(codex: &Arc<CassConversation>) -> Result<UndoCompletedEvent> {
...
async fn expect_successful_undo(codex: &Arc<CassConversation>) -> Result<UndoCompletedEvent> {
...
async fn expect_failed_undo(codex: &Arc<CassConversation>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(codex).await?;
    assert!(
        !event.success,
...
async fn undo_removes_new_file_created_during_turn() -> Result<()> {
    ...
    let codex = Arc::clone(&harness.test().cass);

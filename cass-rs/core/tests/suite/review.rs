...
use cass_core::protocol::RolloutItem;
use cass_core::protocol::RolloutLine;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under Cass sandbox network restrictions.
    skip_if_no_network!();
    ...
    let codex = new_conversation_for_server(&server, &cass_home, |_| {}).await;
    ...
    wait_for_event_with_timeout(
        ...
        |event| match event {
            ...
            EventMsg::ItemCompleted(ev) => match &ev.item {
                cass_protocol::items::TurnItem::AgentMessage(_) => {
                    panic!(
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let codex = new_conversation_for_server(&server, &cass_home, |cfg| {
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = cass_home.path().join("resume.jsonl");
    {
        ...
        let user = cass_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = cass_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let codex =
        resume_conversation_for_server(&server, &cass_home, session_file.clone(), |_| {}).await;

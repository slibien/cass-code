...
fn write_minimal_rollout_with_id(cass_home: &Path, id: Uuid) -> PathBuf {
    let sessions = cass_home.join("sessions/2024/01/01");
...
#[tokio::test]
async fn find_handles_gitignore_covering_cass_home_directory() {
    let repo = TempDir::new().unwrap();
    let cass_home = repo.path().join(".cass");
    std::fs::create_dir_all(&cass_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".cass/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&cass_home, id);

    let found = find_conversation_path_by_id_str(&cass_home, &id.to_string())
        .await
        .unwrap();

...
async fn emits_deprecation_notice_for_legacy_feature_flag() -> anyhow::Result<()> {
    ...
    let TestCass { codex, .. } = builder.build(&server).await?;
    ...
    assert_eq!(
        details.as_deref(),
        Some(
            "Enable it with `--enable streamable_shell` or `[features].streamable_shell` in config.toml. See https://github.com/slibien/cass-code/blob/main/docs/config.md#feature-flags for details."
        ),
    );

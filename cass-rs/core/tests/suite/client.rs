...
use cass_core::ContentItem;
use cass_core::ConversationManager;
use cass_core::LocalShellAction;
use cass_core::LocalShellExecAction;
use cass_core::LocalShellStatus;
use cass_core::ModelClient;
use cass_core::ModelProviderInfo;
use cass_core::NewConversation;
use cass_core::Prompt;
use cass_core::ResponseEvent;
use cass_core::ResponseItem;
use cass_core::WireApi;
use cass_core::auth::AuthCredentialsStoreMode;
use cass_core::built_in_model_providers;
use cass_core::error::CassErr;
use cass_core::model_family::find_family_for_model;
use cass_core::protocol::EventMsg;
use cass_core::protocol::Op;
use cass_core::protocol::SessionSource;
use cass_otel::otel_event_manager::OtelEventManager;
use cass_protocol::ConversationId;
use cass_protocol::models::ReasoningItemContent;
use cass_protocol::models::ReasoningItemReasoningSummary;
use cass_protocol::models::WebSearchAction;
use cass_protocol::user_input::UserInput;
use core_test_support::load_default_config_for_test;
use core_test_support::load_sse_fixture_with_id;
use core_test_support::responses;
use core_test_support::skip_if_no_network;
use core_test_support::test_codex::TestCass;
...
/// Writes an `auth.json` into the provided `cass_home` with the specified parameters.
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = cass_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![cass_protocol::models::ContentItem::InputText {
    ...
    let prior_system = cass_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![cass_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = cass_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Cass to resume from our file
    let model_provider = ModelProviderInfo {
        base_url: Some(format!("{}/v1", server.uri())),
        ..built_in_model_providers()["openai"].clone()
    };
    let cass_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&cass_home);
    config.model_provider = model_provider;
    // Also configure user instructions to ensure they are NOT delivered on resume.
    config.user_instructions = Some("be nice".to_string());

    let conversation_manager =
        ConversationManager::with_auth(CassAuth::from_api_key("Test API Key"));
    let auth_manager =
        cass_core::AuthManager::from_auth_for_testing(CassAuth::from_api_key("Test API Key"));
    let NewConversation {
        conversation: codex,
        session_configured,
        ..
...
async fn includes_conversation_id_and_model_headers_in_request() {
    ...
    assert_eq!(request_originator.to_str().unwrap(), "cass_cli_rs");
    assert_eq!(
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut config = load_default_config_for_test(&cass_home);
    config.model_provider = model_provider;
    let conversation_manager = ConversationManager::with_auth(create_dummy_cass_auth());
    let NewConversation {
        conversation: codex,
        conversation_id,
        session_configured: _,
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth_manager =
        match CassAuth::from_auth_storage(cass_home.path(), AuthCredentialsStoreMode::File) {
            Ok(Some(auth)) => cass_core::AuthManager::from_auth_for_testing(auth),
            Ok(None) => panic!("No CassAuth found in cass_home"),
            Err(e) => panic!("Failed to load CassAuth: {e}"),
        };
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let client = ModelClient::new(
        ...
        summary,
        conversation_id,
        cass_protocol::protocol::SessionSource::Exec,
    );
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let conversation_manager = ConversationManager::with_auth(CassAuth::from_api_key("test"));
    let codex = conversation_manager
        .new_conversation(config)
        .await
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let cass_fixture = builder.build(&server).await?;
    let codex = cass_fixture.cass.clone();
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let expected_context_window_message = CassErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_cass_auth() -> CassAuth {
    CassAuth::create_dummy_chatgpt_auth_for_testing()
}
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Cass sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();

...
async fn model_selects_expected_tools() {
    ...
    let cass_tools = collect_tool_identifiers_for_model("cass-mini-latest").await;
    assert_eq!(
        cass_tools,
        vec![
        ...
        "cass-mini-latest should expose the local shell tool",
    );
    ...
    let gpt5_cass_tools = collect_tool_identifiers_for_model("gpt-5-codex").await;
    assert_eq!(
        gpt5_cass_tools,
        vec![
            "shell".to_string(),

...
use cass_core::protocol::GitInfo;
...
async fn exec_cli_applies_experimental_instructions_file() {
    ...
    // Cass to use the Responses API with the dummy env var.
    let provider_override = format!(
...
async fn integration_git_info_unit_test() {
    ...
    let git_info = cass_core::git_info::collect_git_info(&git_repo).await;

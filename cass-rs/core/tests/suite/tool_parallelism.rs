...
async fn run_turn(test: &TestCass, prompt: &str) -> anyhow::Result<()> {
    ...
    wait_for_event(&test.cass, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;

    Ok(())
}

async fn run_turn_and_measure(test: &TestCass, prompt: &str) -> anyhow::Result<Duration> {
...
async fn build_cass_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestCass> {
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_cass_with_test_tool(&server).await?;

...
use cass_core::auth::AuthDotJson;
use cass_core::auth::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
use cass_core::auth::RefreshTokenError;
use cass_core::auth::load_auth_dot_json;
use cass_core::auth::save_auth;
use cass_core::error::RefreshTokenFailedReason;
use cass_core::token_data::IdTokenInfo;
use cass_core::token_data::TokenData;
...
impl RefreshTokenTestContext {
    fn new(server: &MockServer) -> Result<Self> {
        ...
        let auth = CassAuth::from_auth_storage(cass_home.path(), AuthCredentialsStoreMode::File)?
    ...
    fn load_auth(&self) -> Result<AuthDotJson> {
        load_auth_dot_json(self.cass_home.path(), AuthCredentialsStoreMode::File)

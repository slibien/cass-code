...
use cass_core::protocol::ItemCompletedEvent;
use cass_core::protocol::ItemStartedEvent;
...
async fn user_message_item_is_emitted() -> anyhow::Result<()> {
    ...
    let TestCass { codex, .. } = test_codex().build(&server).await?;
...
async fn assistant_message_item_is_emitted() -> anyhow::Result<()> {
    ...
    let Some(cass_protocol::items::AgentMessageContent::Text { text }) = completed.content.first()
    else {

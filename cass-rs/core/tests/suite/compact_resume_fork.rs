...
//! request payload that Cass would send to the model and assert that the
...
async fn start_test_conversation(
    server: &MockServer,
) -> (TempDir, Config, ConversationManager, Arc<CassConversation>) {
    ...
    let manager = ConversationManager::with_auth(CassAuth::from_api_key("dummy"));
...
async fn user_turn(conversation: &Arc<CassConversation>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<CassConversation>) {
    conversation
        .submit(Op::Compact)
        .await
...
async fn fetch_conversation_path(conversation: &Arc<CassConversation>) -> std::path::PathBuf {
...
async fn resume_conversation(
    ...
    let auth_manager =
        cass_core::AuthManager::from_auth_for_testing(CassAuth::from_api_key("dummy"));
    let NewConversation { conversation, .. } = manager

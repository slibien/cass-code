...
use chrono::Local;
use chrono::Utc;
use cass_async_utils::CancelErr;
use cass_protocol::ConversationId;
use cass_protocol::protocol::RateLimitSnapshot;
...
pub type Result<T> = std::result::Result<T, CassErr>;
...
pub enum CassErr {
    ...
    #[error(
        "Cass ran out of room in the model's context window. Start a new conversation or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `CassError::Spawn` variant.
    ...
    #[error(
        "To use Cass with your ChatGPT plan, upgrade to Plus: https://openai.com/chatgpt/pricing."
    )]
    UsageNotIncluded,
    ...
    #[error("cass-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for CassErr {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let message = match self.plan_type.as_ref() {
            ...
            Some(PlanType::Known(KnownPlan::Free)) => {
                "You've hit your usage limit. Upgrade to Plus to continue using Cass (https://openai.com/chatgpt/pricing)."
                    .to_string()
            }
...
impl CassErr {
    /// Minimal shim so that existing `e.downcast_ref::<CassErr>()` checks continue to compile
...
pub fn get_error_message_ui(e: &CassErr) -> String {
    let message = match e {
        CassErr::Sandbox(SandboxErr::Denied { output }) => {
        ...
        CassErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
...
mod tests {
    ...
    use chrono::TimeZone;
    use chrono::Utc;
    use cass_protocol::protocol::RateLimitWindow;
    ...
    fn sandbox_denied_uses_aggregated_output_when_stderr_empty() {
        ...
        let err = CassErr::Sandbox(SandboxErr::Denied {

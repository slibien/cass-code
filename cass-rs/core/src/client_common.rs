...
use cass_apply_patch::APPLY_PATCH_TOOL_INSTRUCTIONS;
...
use cass_protocol::config_types::Verbosity as VerbosityConfig;
...
pub(crate) fn create_text_param_for_request(
    ...
    Some(TextControls {
        ...
        format: output_schema.as_ref().map(|schema| TextFormat {
            ...
            strict: true,
            schema: schema.clone(),
            name: "cass_output_schema".to_string(),
        }),
    })
}
...
mod tests {
    ...
    fn serializes_text_schema_with_strict_format() {
        ...
        assert_eq!(
            format.get("name"),
            Some(&serde_json::Value::String("cass_output_schema".into()))
        );
        assert_eq!(
            format.get("type"),

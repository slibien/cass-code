...
use crate::default_client::CassHttpClient;
...
pub struct CassAuth {
    ...
    pub(crate) client: CassHttpClient,
}

impl PartialEq for CassAuth {
...
impl CassAuth {
    ...
    ) -> std::io::Result<Option<CassAuth>> {
        load_auth(cass_home, false, auth_credentials_store_mode)
    }
    ...
    fn from_api_key_with_client(api_key: &str, client: CassHttpClient) -> Self {
        Self {
...
pub fn read_cass_api_key_from_env() -> Option<String> {
...
/// Delete the auth.json file inside `cass_home` if it exists. Returns `Ok(true)`
/// if a file was removed, `Ok(false)` if no auth file was present.
pub fn logout(
    cass_home: &Path,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<bool> {
    let storage = create_auth_storage(cass_home.to_path_buf(), auth_credentials_store_mode);
    storage.delete()
}
...
pub fn login_with_api_key(
    ...
    save_auth(cass_home, &auth_dot_json, auth_credentials_store_mode)
}
...
fn logout_with_message(
    ...
    match logout(cass_home, auth_credentials_store_mode) {
...
fn load_auth(
    cass_home: &Path,
    enable_cass_api_key_env: bool,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<Option<CassAuth>> {
    if enable_cass_api_key_env && let Some(api_key) = read_cass_api_key_from_env() {
        let client = crate::default_client::create_client();
        return Ok(Some(CassAuth::from_api_key_with_client(
            api_key.as_str(),
            client,
        )));
    }
    ...
    if let Some(api_key) = &auth_json_api_key {
        return Ok(Some(CassAuth::from_api_key_with_client(api_key, client)));
    }
...
mod tests {
    ...
    fn missing_auth_json_returns_none() {
        let dir = tempdir().unwrap();
        let auth = CassAuth::from_auth_storage(dir.path(), AuthCredentialsStoreMode::File)
    ...
    async fn pro_account_with_no_api_key_uses_chatgpt_auth() {
        ...
        let CassAuth {
            ...
            storage: _,
            ..
        } = super::load_auth(cass_home.path(), false, AuthCredentialsStoreMode::File)
            .unwrap()
            .unwrap();
    ...
    fn write_auth_file(params: AuthFileParams, cass_home: &Path) -> std::io::Result<String> {
        let auth_file = get_auth_file(cass_home);
    ...
    async fn enforce_login_restrictions_logs_out_for_method_mismatch() {
        let cass_home = tempdir().unwrap();
        login_with_api_key(cass_home.path(), "sk-test", AuthCredentialsStoreMode::File)
            .expect("seed api key");

        let config = build_config(cass_home.path(), Some(ForcedLoginMethod::Chatgpt), None);
    ...
    async fn enforce_login_restrictions_logs_out_for_workspace_mismatch() {
        ...
        let config = build_config(cass_home.path(), None, Some("org_mine".to_string()));
...
/// hands out cloned `CassAuth` values so the rest of the program has a
/// consistent snapshot.
///
...
impl AuthManager {
    ...
    /// Create an AuthManager with a specific CassAuth, for testing only.
    pub fn from_auth_for_testing(auth: CassAuth) -> Arc<Self> {
        let cached = CachedAuth { auth: Some(auth) };
        Arc::new(Self {
            cass_home: PathBuf::new(),
            inner: RwLock::new(cached),
            enable_cass_api_key_env: false,
            auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        })
    }

    /// Current cached auth (clone). May be `None` if not logged in or load failed.
    pub fn auth(&self) -> Option<CassAuth> {
    ...
    fn auths_equal(a: &Option<CassAuth>, b: &Option<CassAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => a == b,
            _ => false,
    ...
    pub fn logout(&self) -> std::io::Result<bool> {
        let removed = super::auth::logout(&self.cass_home, self.auth_credentials_store_mode)?;

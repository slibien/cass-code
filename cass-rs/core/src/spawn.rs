...
///
/// 1. The process was spawned by Cass as part of a shell tool call.
...
pub(crate) async fn spawn_child_async(
    ...
    // If this Cass process dies (including being killed via SIGKILL), we want
    ...
    unsafe {
        ...
        cmd.pre_exec(move || {
            ...
            // run _after_ the parent (i.e., the Cass process) has already
            // exited, then parent will be the closest configured "subreaper"
            // ancestor process, or PID 1 (init). If the Cass process has exited

...
//!
//! The history is stored at `~/.cass/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.cass`.
...
fn history_filepath(config: &Config) -> PathBuf {
    let mut path = config.cass_home.clone();
...
pub(crate) async fn append_entry(
    ...
    // Resolve `~/.cass/history.jsonl` and ensure the parent directory exists.

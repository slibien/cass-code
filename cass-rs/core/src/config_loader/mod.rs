...
pub async fn load_config_as_toml(cass_home: &Path) -> io::Result<TomlValue> {
    load_config_as_toml_with_overrides(cass_home, LoaderOverrides::default()).await
}
...
pub(crate) async fn load_config_layers_with_overrides(
    ...
    load_config_layers_internal(cass_home, overrides).await
}
...
async fn load_config_as_toml_with_overrides(
    ...
    let layers = load_config_layers_internal(cass_home, overrides).await?;
...
async fn load_config_layers_internal(
    ...
    let managed_config_path =
        managed_config_path.unwrap_or_else(|| managed_config_default_path(cass_home));

    let user_config_path = cass_home.join(CONFIG_TOML_FILE);
...
fn managed_config_default_path(cass_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = cass_home;
    ...
    #[cfg(not(unix))]
    {
        cass_home.join("managed_config.toml")
    }
}

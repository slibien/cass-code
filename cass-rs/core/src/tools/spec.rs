...
fn create_test_sync_tool() -> ToolSpec {
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description: "Internal synchronization helper used by Cass integration tests.".to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
            required: None,
...
mod tests {
    ...
    fn test_build_specs_contains_expected_basics() {
        let model_family = find_family_for_model("cass-mini-latest")
            .expect("cass-mini-latest should be a valid model family");
        let mut features = Features::with_defaults();
        features.enable(Feature::WebSearchRequest);
        features.enable(Feature::UnifiedExec);
        let config = ToolsConfig::new(&ToolsConfigParams {

...
impl ToolOrchestrator {
        ...
        Rq: ProvidesSandboxRetryData,
    {
        ...
        let otel_user = cass_otel::otel_event_manager::ToolDecisionSource::User;
        let otel_cfg = cass_otel::otel_event_manager::ToolDecisionSource::Config;
        ...
        let initial_attempt = SandboxAttempt {
            ...
            cass_linux_sandbox_exe: turn_ctx.cass_linux_sandbox_exe.as_ref(),
        };
        ...
        match tool.run(req, &initial_attempt, tool_ctx).await {
            ...
            Err(ToolError::Cass(CassErr::Sandbox(SandboxErr::Denied { output }))) => {
                if !tool.escalate_on_failure() {
                    return Err(ToolError::Cass(CassErr::Sandbox(SandboxErr::Denied {
                        output,
                    })));
                }
                ...
                if !tool.should_bypass_approval(approval_policy, already_approved) {
                    ...
                    if let Some(metadata) = req.sandbox_retry_data() {
                        ...
                        let friendly = get_error_message_ui(&CassErr::Sandbox(err));

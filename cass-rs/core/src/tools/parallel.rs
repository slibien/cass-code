...
impl ToolCallRuntime {
    ...
    pub(crate) fn handle_tool_call(
    ...
    ) -> impl std::future::Future<Output = Result<ResponseInputItem, CassErr>> {
        ...
        async move {
            match handle.await {
                ...
                Ok(Err(FunctionCallError::Fatal(message))) => Err(CassErr::Fatal(message)),
                Ok(Err(other)) => Err(CassErr::Fatal(other.to_string())),
                Err(err) => Err(CassErr::Fatal(format!(

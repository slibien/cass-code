...
/// Start an interactive sub-Cass conversation and return IO channels.
///
/// The returned `events_rx` yields non-approval events emitted by the sub-agent.
/// Approval requests are handled via `parent_session` and are not surfaced.
/// The returned `ops_tx` allows the caller to submit additional `Op`s to the sub-agent.
pub(crate) async fn run_cass_conversation_interactive(
    config: Config,
    ...
    let CassSpawnOk { codex, .. } = Cass::spawn(
        config,
        auth_manager,
    ...
    let cass_for_events = Arc::clone(&codex);
    ...
    let cass_for_ops = Arc::clone(&codex);
    tokio::spawn(async move {
        forward_ops(cass_for_ops, rx_ops, cancel_token_ops).await;
    });
...
pub(crate) async fn run_cass_conversation_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    input: Vec<UserInput>,
    parent_session: Arc<Session>,
    ...
    let io = run_cass_conversation_interactive(
        config,
        auth_manager,
        parent_session,
        parent_ctx,
...
async fn await_approval_with_cancel<F>(
    ...
    sub_id: &str,
    cancel_token: &CancellationToken,
) -> cass_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = cass_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            parent_session
                .notify_approval(sub_id, cass_protocol::protocol::ReviewDecision::Abort)
                .await;
            cass_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }

...
use cass_protocol::protocol::FileChange;
use cass_protocol::protocol::HasLegacyEvent;
use cass_protocol::protocol::ItemCompletedEvent;
use cass_protocol::protocol::ItemStartedEvent;
use cass_protocol::protocol::RawResponseItemEvent;
use cass_protocol::protocol::ReviewRequest;
use cass_protocol::protocol::RolloutItem;
use cass_protocol::protocol::SessionSource;
use cass_protocol::protocol::TaskStartedEvent;
use cass_protocol::protocol::TurnAbortReason;
use cass_protocol::protocol::TurnContextItem;
...
use cass_async_utils::OrCancelExt;
use cass_otel::otel_event_manager::OtelEventManager;
use cass_protocol::config_types::ReasoningEffort as ReasoningEffortConfig;
use cass_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
use cass_protocol::models::ContentItem;
use cass_protocol::models::FunctionCallOutputPayload;
use cass_protocol::models::ResponseInputItem;
use cass_protocol::models::ResponseItem;
use cass_protocol::protocol::InitialHistory;
use cass_protocol::user_input::UserInput;
use cass_utils_readiness::Readiness;
use cass_utils_readiness::ReadinessFlag;

pub mod compact;
use self::compact::build_compacted_history;
use self::compact::collect_user_messages;

/// The high-level interface to the Cass system.
...
/// Wrapper returned by [`Cass::spawn`] containing the spawned [`Cass`],
/// the submission id for the initial `ConfigureSession` request and the
/// unique session id.
pub struct CassSpawnOk {
    pub codex: Cass,
...
impl Cass {
    /// Spawn a new [`Cass`] and initialize the session.
    pub async fn spawn(
        config: Config,
        auth_manager: Arc<AuthManager>,
        conversation_history: InitialHistory,
        session_source: SessionSource,
    ) -> CassResult<CassSpawnOk> {
        ...
        // Generate a unique ID for the lifetime of this Cass session.
    ...
    /// Use sparingly: prefer `submit()` so Cass is responsible for generating
    /// unique IDs for each submission.
    pub async fn submit_with_id(&self, sub: Submission) -> CassResult<()> {
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| CassErr::InternalAgentDied)?;
        Ok(())
    }
...
pub(crate) struct TurnContext {
    ...
    pub(crate) cass_linux_sandbox_exe: Option<PathBuf>,
...
impl Session {
    fn make_turn_context(
        ...
        TurnContext {
            ...
            cass_linux_sandbox_exe: config.cass_linux_sandbox_exe.clone(),
    ...
    async fn new(
        ...
        for (alias, feature) in session_configuration.features.legacy_feature_usages() {
            ...
            let details = if alias == canonical {
                ...
                Some(format!(
                    "Enable it with `--enable {canonical}` or `[features].{canonical}` in config.toml. See https://github.com/slibien/cass-code/blob/main/docs/config.md#feature-flags for details."
                ))
            };
            post_session_configured_events.push(Event {
                id: INITIAL_SUBMIT_ID.to_owned(),
...
mod handlers {
    ...
    use cass_protocol::custom_prompts::CustomPrompt;
    use cass_protocol::protocol::ErrorEvent;
    use cass_protocol::protocol::Event;
    use cass_protocol::protocol::EventMsg;
    use cass_protocol::protocol::ListCustomPromptsResponseEvent;
    use cass_protocol::protocol::Op;
    use cass_protocol::protocol::ReviewDecision;
    use cass_protocol::protocol::ReviewRequest;
    use cass_protocol::protocol::TurnAbortReason;
    use cass_protocol::user_input::UserInput;
    use std::sync::Arc;
            ...
            let event = Event {
                ...
                msg: EventMsg::GetHistoryEntryResponse(
                    crate::protocol::GetHistoryEntryResponseEvent {
                        offset,
                        log_id,
                        entry: entry_opt.map(|e| cass_protocol::message_history::HistoryEntry {
    ...
    pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
        ...
        info!("Shutting down Cass instance");
...
async fn spawn_review_thread(
    ...
    let review_turn_context = TurnContext {
        ...
        final_output_json_schema: None,
        cass_linux_sandbox_exe: parent_turn_context.cass_linux_sandbox_exe.clone(),
        tool_call_gate: Arc::new(ReadinessFlag::new()),
    };
    ...
    loop {
        ...
        .await
        {
            ...
            Err(CassErr::TurnAborted {
                ...
                return Err(CassErr::TurnAborted {
                    dangling_artifacts: processed_items,
                });
            }
            Err(CassErr::Interrupted) => return Err(CassErr::Interrupted),
            Err(CassErr::EnvVar(var)) => return Err(CassErr::EnvVar(var)),
            Err(e @ CassErr::Fatal(_)) => return Err(e),
            Err(e @ CassErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(e);
            }
            Err(CassErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, rate_limits).await;
                }
                return Err(CassErr::UsageLimitReached(e));
            }
            Err(CassErr::UsageNotIncluded) => return Err(CassErr::UsageNotIncluded),
            Err(e @ CassErr::RefreshTokenFailed(_)) => return Err(e),
            Err(e) => {
                ...
                if retries < max_retries {
                    retries += 1;
                    let delay = match e {
                        CassErr::Stream(_, Some(delay)) => delay,
                        _ => backoff(retries),
                    };
                    warn!(
...
async fn try_run_turn(
    ...
    let mut output: FuturesOrdered<BoxFuture<CassResult<ProcessedResponseItem>>> =
        FuturesOrdered::new();
    ...
    loop {
        ...
        let event = match stream.next().or_cancel(&cancellation_token).await {
            Ok(event) => event,
            Err(cass_async_utils::CancelErr::Cancelled) => {
            ...
            ResponseEvent::OutputItemDone(item) => {
                ...
                match ToolRouter::build_tool_call(sess.as_ref(), item.clone()) {
                    ...
                    Err(FunctionCallError::Fatal(message)) => {
                        return Err(CassErr::Fatal(message));
                    }
                }
            }
...
mod tests {
    ...
    use cass_protocol::models::ContentItem;
    use cass_protocol::models::ResponseItem;
    use cass_protocol::protocol::McpAuthStatus;
    ...
    pub(crate) fn make_session_and_context() -> (Session, TurnContext) {
        ...
        let cass_home = tempfile::tempdir().expect("create temp dir");
        let config = Config::load_from_base_config_with_overrides(
            ConfigToml::default(),
            ConfigOverrides::default(),
            cass_home.path().to_path_buf(),
        )

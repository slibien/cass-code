...
use cass_protocol::models::FunctionCallOutputContentItem;
...
use cass_protocol::protocol::SubAgentSource;
...
pub(crate) async fn stream_chat_completions(
    ...
    if prompt.output_schema.is_some() {
        return Err(CassErr::UnsupportedOperation(
        ...
        match res {
            ...
            Ok(res) => {
                ...
                if !(status == StatusCode::TOO_MANY_REQUESTS || status.is_server_error()) {
                    ...
                    return Err(CassErr::UnexpectedStatus(UnexpectedResponseError {
                        status,
                        body,
                        request_id: None,
                    }));
                ...
                if attempt > max_retries {
                    return Err(CassErr::RetryLimit(RetryLimitReachedError {
                        status,
                        request_id: None,
                    }));
                }
            ...
            Err(e) => {
                if attempt > max_retries {
                    return Err(CassErr::ConnectionFailed(ConnectionFailedError {
                        source: e,
                    }));
                }
...
/// output is mapped onto Cass's internal [`ResponseEvent`] so that the rest
    ...
    S: Stream<Item = Result<Bytes>> + Unpin,
{
    ...
    loop {
        ...
        let sse = match response {
            ...
            Ok(Some(Err(e))) => {
                let _ = tx_event
                    .send(Err(CassErr::Stream(e.to_string(), None)))
                    .await;
                return;
            }
            Ok(None) => {
            ...
            match Pin::new(&mut this.inner).poll_next(cx) {
                ...
                Poll::Ready(Some(Ok(ResponseEvent::OutputItemDone(item)))) => {
                    ...
                    let is_assistant_message = matches!(
                        &item,
                        cass_protocol::models::ResponseItem::Message { role, .. } if role == "assistant"
                    );
                        ...
                        match this.mode {
                            AggregateMode::AggregatedOnly => {
                                ...
                                if this.cumulative.is_empty()
                                    && let cass_protocol::models::ResponseItem::Message {
                                        content,
                                        ..
                                    } = &item
                                    && let Some(text) = content.iter().find_map(|c| match c {
                                        cass_protocol::models::ContentItem::OutputText {
                                            text,
                                        } => Some(text),
                                        _ => None,
                                    })
                ...
                Poll::Ready(Some(Ok(ResponseEvent::Completed {
                        ...
                        && matches!(this.mode, AggregateMode::AggregatedOnly)
                    {
                        let aggregated_reasoning =
                            cass_protocol::models::ResponseItem::Reasoning {
                                id: String::new(),
                                summary: Vec::new(),
                                content: Some(vec![
                                    cass_protocol::models::ReasoningItemContent::ReasoningText {
                    ...
                    if !this.cumulative.is_empty() {
                        let aggregated_message = cass_protocol::models::ResponseItem::Message {
                            id: None,
                            role: "assistant".to_string(),

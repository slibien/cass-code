...
/// See https://github.com/slibien/cass-code/pull/3388/files for an example of what that would look like.
...
/// Parenthesis will be added by Cass. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "cass_cli_rs";
pub const CODEX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "CODEX_INTERNAL_ORIGINATOR_OVERRIDE";

#[derive(Clone, Debug)]
pub struct CassHttpClient {
...
impl CassHttpClient {
    ...
    pub fn get<U>(&self, url: U) -> CassRequestBuilder
    where
        U: IntoUrl,
    {
    ...
    pub fn post<U>(&self, url: U) -> CassRequestBuilder
    where
        U: IntoUrl,
    {
    ...
    pub fn request<U>(&self, method: Method, url: U) -> CassRequestBuilder
    where
        U: IntoUrl,
    {
        let url_str = url.as_str().to_string();
        CassRequestBuilder::new(self.inner.request(method.clone(), url), method, url_str)
    }
}

#[must_use = "requests are not sent unless `send` is awaited"]
#[derive(Debug)]
pub struct CassRequestBuilder {
    builder: reqwest::RequestBuilder,
    method: Method,
    url: String,
}

impl CassRequestBuilder {
...
pub fn get_cass_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Cass user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Cass user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Cass originator because base user agent string is invalid"
        );
...
pub fn create_client() -> CassHttpClient {
    use reqwest::header::HeaderMap;

    let mut headers = HeaderMap::new();
    headers.insert("originator", originator().header_value.clone());
    let ua = get_cass_user_agent();
...
mod tests {
    ...
    #[test]
    fn test_get_cass_user_agent() {
        let user_agent = get_cass_user_agent();
        assert!(user_agent.starts_with("cass_cli_rs/"));
    }
    ...
    async fn test_create_client_sets_default_headers() {
        ...
        assert_eq!(originator_header.to_str().unwrap(), "cass_cli_rs");

        // User-Agent matches the computed Cass UA for that originator
        let expected_ua = get_cass_user_agent();
    ...
    fn test_invalid_suffix_is_sanitized() {
        let prefix = "cass_cli_rs/0.0.0";
        let suffix = "bad\rsuffix";

        assert_eq!(
            sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
            "cass_cli_rs/0.0.0 (bad_suffix)"
        );
    }
    ...
    fn test_macos() {
        ...
        let user_agent = get_cass_user_agent();
        let re = Regex::new(
            r"^cass_cli_rs/\d+\.\d+\.\d+ \(Mac OS \d+\.\d+\.\d+; (x86_64|arm64)\) (\S+)$",
        )
        .unwrap();

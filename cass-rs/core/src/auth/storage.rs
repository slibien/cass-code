...
use cass_keyring_store::DefaultKeyringStore;
use cass_keyring_store::KeyringStore;

/// Determine where Cass should store CLI auth credentials.
...
pub(super) fn get_auth_file(cass_home: &Path) -> PathBuf {
    cass_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(cass_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(cass_home: PathBuf) -> Self {
        Self { cass_home }
    }
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.cass_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.cass_home)
    }
}

const KEYRING_SERVICE: &str = "Cass Auth";

// turns cass_home path into a stable, short key string
fn compute_store_key(cass_home: &Path) -> std::io::Result<String> {
    let canonical = cass_home
        .canonicalize()
        .unwrap_or_else(|_| cass_home.to_path_buf());
...
impl KeyringAuthStorage {
    fn new(cass_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            cass_home,
            keyring_store,
        }
...
impl AuthStorageBackend for KeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.cass_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.cass_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.cass_home)?;
...
impl AutoAuthStorage {
    fn new(cass_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            keyring_storage: Arc::new(KeyringAuthStorage::new(cass_home.clone(), keyring_store)),
            file_storage: Arc::new(FileAuthStorage::new(cass_home)),
        }
    }
}
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_keyring_store(cass_home, mode, keyring_store)
}
...
fn create_auth_storage_with_keyring_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(cass_home)),
        AuthCredentialsStoreMode::Keyring => {
            Arc::new(KeyringAuthStorage::new(cass_home, keyring_store))
        }
        AuthCredentialsStoreMode::Auto => Arc::new(AutoAuthStorage::new(cass_home, keyring_store)),
    }
}
...
mod tests {
    ...
    use cass_keyring_store::tests::MockKeyringStore;
    use keyring::Error as KeyringError;

    #[tokio::test]
    async fn file_storage_load_returns_auth_dot_json() -> anyhow::Result<()> {
        let cass_home = tempdir()?;
        let storage = FileAuthStorage::new(cass_home.path().to_path_buf());
    ...
    async fn file_storage_save_persists_auth_dot_json() -> anyhow::Result<()> {
        ...
        let file = get_auth_file(cass_home.path());
        storage
            .save(&auth_dot_json)
            .context("failed to save auth file")?;
    ...
    fn keyring_auth_storage_load_returns_deserialized_auth() -> anyhow::Result<()> {
        ...
        seed_keyring_with_auth(
            &mock_keyring,
            || compute_store_key(cass_home.path()),
            &expected,
        )?;
    ...
    fn keyring_auth_storage_compute_store_key_for_home_directory() -> anyhow::Result<()> {
        let cass_home = PathBuf::from("~/.cass");

        let key = compute_store_key(cass_home.as_path())?;
    ...
    fn keyring_auth_storage_save_persists_and_removes_fallback_file() -> anyhow::Result<()> {
        ...
        let auth_file = get_auth_file(cass_home.path());
        std::fs::write(&auth_file, "stale")?;
        ...
        let key = compute_store_key(cass_home.path())?;
    ...
    fn auto_auth_storage_load_uses_file_when_keyring_empty() -> anyhow::Result<()> {
        let cass_home = tempdir()?;
        let mock_keyring = MockKeyringStore::default();
        let storage = AutoAuthStorage::new(cass_home.path().to_path_buf(), Arc::new(mock_keyring));

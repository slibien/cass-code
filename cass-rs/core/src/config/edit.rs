...
pub fn apply_blocking(
    ...
    let config_path = cass_home.join(CONFIG_TOML_FILE);
    ...
    std::fs::create_dir_all(cass_home).with_context(|| {
        format!(
            "failed to create Cass home directory at {}",
            cass_home.display()
        )
    })?;

    let tmp = NamedTempFile::new_in(cass_home)?;
...
pub async fn apply(
    ...
    let cass_home = cass_home.to_path_buf();
    let profile = profile.map(ToOwned::to_owned);
    task::spawn_blocking(move || apply_blocking(&cass_home, profile.as_deref(), &edits))
        .await
...
impl ConfigEditsBuilder {
    pub fn new(cass_home: &Path) -> Self {
        Self {
            cass_home: cass_home.to_path_buf(),
            profile: None,
            edits: Vec::new(),
        }
    }
    ...
    pub fn apply_blocking(self) -> anyhow::Result<()> {
        apply_blocking(&self.cass_home, self.profile.as_deref(), &self.edits)
    }
...
mod tests {
    ...
    fn blocking_set_model_top_level() {
        ...
        let contents =
            std::fs::read_to_string(cass_home.join(CONFIG_TOML_FILE)).expect("read config");
...
"#;
    ...
    fn blocking_set_model_preserves_inline_table_contents() {
        ...
        std::fs::write(
            cass_home.join(CONFIG_TOML_FILE),
            r#"profile = "fast"
...
"#,
        ...
        let raw = std::fs::read_to_string(cass_home.join(CONFIG_TOML_FILE)).expect("read config");
...
";
    ...
    fn blocking_clear_path_noop_when_missing() {
        ...
        assert!(
            !cass_home.join(CONFIG_TOML_FILE).exists(),
    ...
    async fn async_builder_set_model_persists() {
        let tmp = tempdir().expect("tmpdir");
        let cass_home = tmp.path().to_path_buf();
...
"#;
        ...
        contents = std::fs::read_to_string(cass_home.join(CONFIG_TOML_FILE)).expect("read config");

...
use cass_app_server_protocol::Tools;
use cass_app_server_protocol::UserSavedConfig;
use cass_protocol::config_types::ForcedLoginMethod;
use cass_protocol::config_types::ReasoningEffort;
use cass_protocol::config_types::ReasoningSummary;
use cass_protocol::config_types::SandboxMode;
use cass_protocol::config_types::Verbosity;
use cass_rmcp_client::OAuthCredentialsStoreMode;
use dirs::home_dir;
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, Cass will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Cass
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.cass/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Cass"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Cass '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Cass home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Cass can reach out to for tool calls.
    pub mcp_servers: HashMap<String, McpServerConfig>,

    /// Preferred store for MCP OAuth credentials.
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Cass unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/slibien/cass-code/blob/main/codex-rs/rmcp-client/src/oauth.rs#L2
    /// file: CODEX_HOME/.credentials.json
    ///       This file will be readable to Cass and other applications running as the same user.
    ...
    /// Directory containing all Cass state (defaults to `~/.cass` but can be
    /// overridden by the `CODEX_HOME` environment variable).
    pub cass_home: PathBuf,

    /// Settings that govern if and what will be written to `~/.cass/history.jsonl`.
    pub history: History,
    ...
    pub cass_linux_sandbox_exe: Option<PathBuf>,
...
impl Config {
    pub async fn load_with_cli_overrides(
        ...
        Self::load_from_base_config_with_overrides(cfg, overrides, cass_home)
    }
}
...
async fn load_resolved_config(
    ...
    let layers = load_config_layers_with_overrides(cass_home, overrides).await?;
...
pub async fn load_global_mcp_servers(
    ...
    let root_value = load_config_as_toml(cass_home).await?;
...
/// Use with caution.
pub fn set_project_trusted(cass_home: &Path, project_path: &Path) -> anyhow::Result<()> {
    use crate::config::edit::ConfigEditsBuilder;

    ConfigEditsBuilder::new(cass_home)
...
/// Base config deserialized from ~/.cass/config.toml.
...
pub struct ConfigToml {
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          https://github.com/slibien/cass-code/blob/main/codex-rs/rmcp-client/src/oauth.rs#L2
    /// file: Use a file in the Cass home directory.
...
impl Config {
    ...
    pub fn load_from_base_config_with_overrides(
        ...
        let user_instructions = Self::load_instructions(Some(&cass_home));
    ...
    fn load_instructions(cass_dir: Option<&Path>) -> Option<String> {
        let base = cass_dir?;
...
/// Returns the path to the Cass configuration directory, which can be
...
///   directory exists.
pub fn find_cass_home() -> std::io::Result<PathBuf> {
...
/// Returns the path to the folder where Cass logs are stored. Does not verify
/// that the directory exists.
pub fn log_dir(cfg: &Config) -> std::io::Result<PathBuf> {
    let mut p = cfg.cass_home.clone();
    p.push("log");
    Ok(p)
}
...
"#;
    ...
    async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
        let cass_home = TempDir::new()?;
        let managed_path = cass_home.path().join("managed_config.toml");
        let config_path = cass_home.path().join(CONFIG_TOML_FILE);
        ...
        let root_value = load_resolved_config(cass_home.path(), Vec::new(), overrides).await?;
        let cfg: ConfigToml = root_value.try_into().map_err(|e| {
            tracing::error!("Failed to deserialize overridden config: {e}");
            std::io::Error::new(std::io::ErrorKind::InvalidData, e)
        })?;
    ...
    async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
        ...
        let servers = load_global_mcp_servers(cass_home.path()).await?;
    ...
    async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
        ...
        let loaded = load_global_mcp_servers(cass_home.path()).await?;
    ...
    async fn managed_config_wins_over_cli_overrides() -> anyhow::Result<()> {
        ...
        std::fs::write(
            cass_home.path().join(CONFIG_TOML_FILE),
            "model = \"base\"\n",
        )?;
...
"#,
        ...
        let err = load_global_mcp_servers(cass_home.path())
            .await
...
"#
    ...
    async fn set_model_updates_defaults() -> anyhow::Result<()> {
        ...
        ConfigEditsBuilder::new(cass_home.path())
            .set_model(Some("gpt-5-codex"), Some(ReasoningEffort::High))
            .apply()
            .await?;

        let serialized =
            tokio::fs::read_to_string(cass_home.path().join(CONFIG_TOML_FILE)).await?;
...
"#,
    ...
    impl PrecedenceTestFixture {
        ...
        fn cass_home(&self) -> PathBuf {
            self.cass_home.path().to_path_buf()
        }
    }
    ...
    fn loads_compact_prompt_from_file() -> std::io::Result<()> {
        let cass_home = TempDir::new()?;
        let workspace = cass_home.path().join("workspace");
...
"#;
        ...
        let cass_home_temp_dir = TempDir::new().unwrap();
    ...
    fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
        ...
        assert_eq!(
            Config {
                ...
                cass_home: fixture.cass_home(),
                history: History::default(),
                file_opener: UriBasedFileOpener::VsCode,
                cass_linux_sandbox_exe: None,

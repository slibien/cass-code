...
use crate::codex::CassSpawnOk;
use crate::codex::INITIAL_SUBMIT_ID;
use crate::cass_conversation::CassConversation;
...
/// Represents a newly created Cass conversation, including the first event
/// (which is [`EventMsg::SessionConfigured`]).
pub struct NewConversation {
    pub conversation_id: ConversationId,
    pub conversation: Arc<CassConversation>,
...
pub struct ConversationManager {
    conversations: Arc<RwLock<HashMap<ConversationId, Arc<CassConversation>>>>,
...
impl ConversationManager {
    ...
    /// Construct with a dummy AuthManager containing the provided CassAuth.
    /// Used for integration tests: should not be used by ordinary business logic.
    pub fn with_auth(auth: CassAuth) -> Self {
        Self::new(
    ...
    pub async fn new_conversation(&self, config: Config) -> CassResult<NewConversation> {
    ...
    async fn finalize_spawn(
        ...
        let session_configured = match event {
            ...
            _ => {
                return Err(CassErr::SessionConfiguredNotFirstEvent);
            }
        };

        let conversation = Arc::new(CassConversation::new(
            codex,
    ...
    pub async fn get_conversation(
        &self,
        conversation_id: ConversationId,
    ) -> CassResult<Arc<CassConversation>> {
        let conversations = self.conversations.read().await;
        conversations
            .get(&conversation_id)
            .cloned()
            .ok_or_else(|| CassErr::ConversationNotFound(conversation_id))
    }
    ...
    /// conversation is stored as `Arc<CassConversation>`, it is possible that
    ...
    pub async fn fork_conversation(
        ...
        let CassSpawnOk {
            codex,
            conversation_id,
        } = Cass::spawn(config, auth_manager, history, self.session_source.clone()).await?;

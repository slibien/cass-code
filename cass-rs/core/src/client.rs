        ...
        match res {
            Ok(resp) if resp.status().is_success() => {
                ...
                let stream = resp.bytes_stream().map_err(move |e| {
                    CassErr::ResponseStreamFailed(ResponseStreamFailed {
                        source: e,
            ...
            Ok(res) => {
                    ...
                    && let Err(err) = manager.refresh_token().await
                {
                    let stream_error = match err {
                        RefreshTokenError::Permanent(failed) => {
                            StreamAttemptError::Fatal(CassErr::RefreshTokenFailed(failed))
                        }
                        RefreshTokenError::Transient(other) => {
                            StreamAttemptError::RetryableTransportError(CassErr::Io(other))
                        }
                    };
                    ...
                    || status.is_server_error())
                {
                    ...
                    return Err(StreamAttemptError::Fatal(CassErr::UnexpectedStatus(
                ...
                if status == StatusCode::TOO_MANY_REQUESTS {
                    ...
                    if let Some(ErrorResponse { error }) = body {
                        if error.r#type.as_deref() == Some("usage_limit_reached") {
                            ...
                            let plan_type = error
                                .plan_type
                                .or_else(|| auth.as_ref().and_then(CassAuth::get_plan_type));
                            let resets_at = error
                                .resets_at
                                .and_then(|seconds| DateTime::<Utc>::from_timestamp(seconds, 0));
                            let cass_err = CassErr::UsageLimitReached(UsageLimitReachedError {
                                plan_type,
                                resets_at,
                                rate_limits: rate_limit_snapshot,
                            });
                            return Err(StreamAttemptError::Fatal(cass_err));
                        } else if error.r#type.as_deref() == Some("usage_not_included") {
                            return Err(StreamAttemptError::Fatal(CassErr::UsageNotIncluded));
                        }
                    }
                }
            ...
            Err(e) => Err(StreamAttemptError::RetryableTransportError(
                CassErr::ConnectionFailed(ConnectionFailedError { source: e }),
            )),
        }
    }
...
enum StreamAttemptError {
    ...
    RetryableTransportError(CassErr),
    Fatal(CassErr),
}
...
impl StreamAttemptError {
    ...
    fn into_error(self) -> CassErr {
        match self {
            Self::RetryableHttpError {
                status, request_id, ..
            } => {
                if status == StatusCode::INTERNAL_SERVER_ERROR {
                    CassErr::InternalServerError
                } else {
                    CassErr::RetryLimit(RetryLimitReachedError { status, request_id })
                }
            }
    ...
    S: Stream<Item = Result<Bytes>> + Unpin,
{
    ...
    let mut response_error: Option<CassErr> = None;
    ...
    loop {
        ...
        let sse = match response {
            ...
            Ok(Some(Err(e))) => {
                debug!("SSE Error: {e:#}");
                let event = CassErr::Stream(e.to_string(), None);
            ...
            Ok(None) => {
                match response_completed {
                    ...
                    None => {
                        let error = response_error.unwrap_or(CassErr::Stream(
        ...
        match event.kind.as_str() {
            ...
            "response.failed" => {
                if let Some(resp_val) = event.response {
                    response_error = Some(CassErr::Stream(
                        ...
                        match serde_json::from_value::<Error>(error.clone()) {
                            Ok(error) => {
                                if is_context_window_error(&error) {
                                    response_error = Some(CassErr::ContextWindowExceeded);
                                } else {
                                    let delay = try_parse_retry_after(&error);
                                    let message = error.message.clone().unwrap_or_default();
                                    response_error = Some(CassErr::Stream(message, delay));
                                }
                            }
                            Err(e) => {
                                let error = format!("failed to parse ErrorResponse: {e}");
                                debug!(error);
                                response_error = Some(CassErr::Stream(error, None))
                            }
                        }
                    }
                }
...
async fn stream_from_fixture(
    ...
    let stream = ReaderStream::new(rdr).map_err(CassErr::Io);
    tokio::spawn(process_sse(
        stream,
        tx_event,
...
mod tests {
    ...
    async fn collect_events(
        ...
        let stream = ReaderStream::new(reader).map_err(CassErr::Io);
    ...
    async fn run_sse(
        ...
        let stream = ReaderStream::new(std::io::Cursor::new(body)).map_err(CassErr::Io);
        tokio::spawn(process_sse(
            stream,
            tx,
            provider.stream_idle_timeout(),
    ...
    async fn context_window_error_is_fatal() {
        ...
        match &events[0] {
            Err(err @ CassErr::ContextWindowExceeded) => {
                assert_eq!(err.to_string(), CassErr::ContextWindowExceeded.to_string());
            }

...
use cass_core::config::find_cass_home;
use cass_core::config::load_global_mcp_servers;
...
use cass_core::mcp::auth::compute_auth_statuses;
use cass_core::protocol::McpAuthStatus;
use cass_rmcp_client::delete_oauth_tokens;
use cass_rmcp_client::perform_oauth_login;
use cass_rmcp_client::supports_oauth_login;

/// [experimental] Launch Cass as an MCP server or manage configured MCP servers.
///
/// Subcommands:
/// - `serve`  — run the MCP server on stdio
/// - `list`   — list configured servers (with `--json`)
/// - `get`    — show a single server (with `--json`)
/// - `add`    — add a server launcher entry to `~/.cass/config.toml`
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let cass_home = find_cass_home().context("failed to resolve CODEX_HOME")?;
    let mut servers = load_global_mcp_servers(&cass_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", cass_home.display()))?;
    ...
    ConfigEditsBuilder::new(&cass_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", cass_home.display()))?;
...
async fn run_login(config_overrides: &CliConfigOverrides, login_args: LoginArgs) -> Result<()> {
    ...
    if !config.features.enabled(Feature::RmcpClient) {
        bail!(
            "OAuth login is only supported when [features].rmcp_client is true in config.toml. See https://github.com/slibien/cass-code/blob/main/docs/config.md#feature-flags for details."
        );
    }

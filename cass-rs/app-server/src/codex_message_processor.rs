...
use cass_app_server_protocol::AccountLoginCompletedNotification;
use cass_app_server_protocol::AccountRateLimitsUpdatedNotification;
use cass_app_server_protocol::AccountUpdatedNotification;
use cass_app_server_protocol::AddConversationListenerParams;
use cass_app_server_protocol::AddConversationSubscriptionResponse;
use cass_app_server_protocol::ApplyPatchApprovalParams;
use cass_app_server_protocol::ApplyPatchApprovalResponse;
use cass_app_server_protocol::ArchiveConversationParams;
use cass_app_server_protocol::ArchiveConversationResponse;
use cass_app_server_protocol::AuthMode;
use cass_app_server_protocol::AuthStatusChangeNotification;
use cass_app_server_protocol::CancelLoginAccountParams;
use cass_app_server_protocol::CancelLoginAccountResponse;
use cass_app_server_protocol::CancelLoginChatGptResponse;
use cass_app_server_protocol::ClientRequest;
use cass_app_server_protocol::ConversationSummary;
use cass_app_server_protocol::ExecCommandApprovalParams;
use cass_app_server_protocol::ExecCommandApprovalResponse;
use cass_app_server_protocol::ExecOneOffCommandParams;
use cass_app_server_protocol::ExecOneOffCommandResponse;
use cass_app_server_protocol::FeedbackUploadParams;
use cass_app_server_protocol::FeedbackUploadResponse;
use cass_app_server_protocol::FuzzyFileSearchParams;
use cass_app_server_protocol::FuzzyFileSearchResponse;
use cass_app_server_protocol::GetAccountRateLimitsResponse;
use cass_app_server_protocol::GetConversationSummaryParams;
use cass_app_server_protocol::GetConversationSummaryResponse;
use cass_app_server_protocol::GetUserAgentResponse;
use cass_app_server_protocol::GetUserSavedConfigResponse;
use cass_app_server_protocol::GitDiffToRemoteResponse;
use cass_app_server_protocol::InputItem as WireInputItem;
use cass_app_server_protocol::InterruptConversationParams;
use cass_app_server_protocol::InterruptConversationResponse;
use cass_app_server_protocol::JSONRPCErrorError;
use cass_app_server_protocol::ListConversationsParams;
use cass_app_server_protocol::ListConversationsResponse;
use cass_app_server_protocol::LoginAccountParams;
use cass_app_server_protocol::LoginApiKeyParams;
use cass_app_server_protocol::LoginApiKeyResponse;
use cass_app_server_protocol::LoginChatGptCompleteNotification;
use cass_app_server_protocol::LoginChatGptResponse;
use cass_app_server_protocol::ModelListParams;
use cass_app_server_protocol::ModelListResponse;
use cass_app_server_protocol::NewConversationParams;
use cass_app_server_protocol::NewConversationResponse;
use cass_app_server_protocol::RemoveConversationListenerParams;
use cass_app_server_protocol::RemoveConversationSubscriptionResponse;
use cass_app_server_protocol::RequestId;
use cass_app_server_protocol::Result as JsonRpcResult;
use cass_app_server_protocol::ResumeConversationParams;
use cass_app_server_protocol::SendUserMessageParams;
use cass_app_server_protocol::SendUserMessageResponse;
use cass_app_server_protocol::SendUserTurnParams;
use cass_app_server_protocol::SendUserTurnResponse;
use cass_app_server_protocol::ServerNotification;
use cass_app_server_protocol::ServerRequestPayload;
use cass_app_server_protocol::SessionConfiguredNotification;
use cass_app_server_protocol::SetDefaultModelParams;
use cass_app_server_protocol::SetDefaultModelResponse;
use cass_app_server_protocol::Thread;
use cass_app_server_protocol::ThreadArchiveParams;
use cass_app_server_protocol::ThreadArchiveResponse;
use cass_app_server_protocol::ThreadListParams;
use cass_app_server_protocol::ThreadListResponse;
use cass_app_server_protocol::ThreadResumeParams;
use cass_app_server_protocol::ThreadResumeResponse;
use cass_app_server_protocol::ThreadStartParams;
use cass_app_server_protocol::ThreadStartResponse;
use cass_app_server_protocol::ThreadStartedNotification;
use cass_app_server_protocol::UserInfoResponse;
use cass_app_server_protocol::UserSavedConfig;
use cass_backend_client::Client as BackendClient;
use cass_core::AuthManager;
use cass_core::CassConversation;
use cass_core::ConversationManager;
use cass_core::Cursor as RolloutCursor;
use cass_core::INTERACTIVE_SESSION_SOURCES;
use cass_core::InitialHistory;
use cass_core::NewConversation;
use cass_core::RolloutRecorder;
use cass_core::SessionMeta;
use cass_core::auth::CLIENT_ID;
use cass_core::auth::login_with_api_key;
use cass_core::config::Config;
use cass_core::config::ConfigOverrides;
use cass_core::config::ConfigToml;
use cass_core::config::edit::ConfigEditsBuilder;
use cass_core::config_loader::load_config_as_toml;
use cass_core::default_client::get_cass_user_agent;
use cass_core::exec::ExecParams;
use cass_core::exec_env::create_env;
use cass_core::find_conversation_path_by_id_str;
use cass_core::get_platform_sandbox;
use cass_core::git_info::git_diff_to_remote;
use cass_core::parse_cursor;
use cass_core::protocol::ApplyPatchApprovalRequestEvent;
use cass_core::protocol::Event;
use cass_core::protocol::EventMsg;
use cass_core::protocol::ExecApprovalRequestEvent;
use cass_core::protocol::Op;
use cass_core::protocol::ReviewDecision;
use cass_core::read_head_for_summary;
use cass_feedback::CassFeedback;
use cass_login::ServerOptions as LoginServerOptions;
use cass_login::ShutdownHandle;
use cass_login::run_login_server;
use cass_protocol::ConversationId;
use cass_protocol::config_types::ForcedLoginMethod;
use cass_protocol::items::TurnItem;
use cass_protocol::models::ResponseItem;
use cass_protocol::protocol::RateLimitSnapshot as CoreRateLimitSnapshot;
use cass_protocol::protocol::RolloutItem;
use cass_protocol::protocol::USER_MESSAGE_BEGIN;
use cass_protocol::user_input::UserInput as CoreInputItem;
use cass_utils_json_to_toml::json_to_toml;
...
/// Handles JSON-RPC messages for Cass conversations.
pub(crate) struct CassMessageProcessor {
...
impl CassMessageProcessor {
    ...
    async fn login_api_key_v2(&mut self, request_id: RequestId, params: LoginApiKeyParams) {
        match self.login_api_key_common(&params).await {
            Ok(()) => {
                let response = cass_app_server_protocol::LoginAccountResponse::ApiKey {};
        ...
        match self.login_chatgpt_common().await {
            Ok(opts) => match run_login_server(opts) {
                Ok(server) => {
                    ...
                    let response = cass_app_server_protocol::LoginAccountResponse::Chatgpt {
        ...
        match self.logout_common().await {
            Ok(current_auth_method) => {
                self.outgoing
                    .send_response(
                        request_id,
                        cass_app_server_protocol::LogoutChatGptResponse {},
                    )
                    .await;
        ...
        match self.logout_common().await {
            Ok(current_auth_method) => {
                self.outgoing
                    .send_response(
                        request_id,
                        cass_app_server_protocol::LogoutAccountResponse {},
                    )
                    .await;
    ...
    async fn get_auth_status(
        ...
        params: cass_app_server_protocol::GetAuthStatusParams,
    ) {
        ...
        let response = if !requires_openai_auth {
            cass_app_server_protocol::GetAuthStatusResponse {
                auth_method: None,
                auth_token: None,
            ...
            match self.auth_manager.auth() {
                ...
                None => cass_app_server_protocol::GetAuthStatusResponse {
                    auth_method: None,
                    auth_token: None,
                    requires_openai_auth: Some(true),
                },
    ...
    async fn fetch_account_rate_limits(&self) -> Result<CoreRateLimitSnapshot, JSONRPCErrorError> {
        let Some(auth) = self.auth_manager.auth() else {
            return Err(JSONRPCErrorError {
                code: INVALID_REQUEST_ERROR_CODE,
                message: "cass account authentication required to read rate limits".to_string(),
                data: None,
            });
        };
    ...
    async fn get_user_saved_config(&self, request_id: RequestId) {
        let toml_value = match load_config_as_toml(&self.config.cass_home).await {
            Ok(val) => val,
            Err(err) => {
                let error = JSONRPCErrorError {
                    code: INTERNAL_ERROR_CODE,
    ...
    async fn set_default_model(&self, request_id: RequestId, params: SetDefaultModelParams) {
        ...
        match ConfigEditsBuilder::new(&self.config.cass_home)
    ...
    async fn exec_one_off_command(&self, request_id: RequestId, params: ExecOneOffCommandParams) {
        ...
        let sandbox_type = match &effective_policy {
            cass_core::protocol::SandboxPolicy::DangerFullAccess => {
                cass_core::exec::SandboxType::None
            }
            _ => get_platform_sandbox().unwrap_or(cass_core::exec::SandboxType::None),
        };
        tracing::debug!("Sandbox type: {sandbox_type:?}");
        let cass_linux_sandbox_exe = self.config.cass_linux_sandbox_exe.clone();
        let outgoing = self.outgoing.clone();
        let req_id = request_id;
        let sandbox_cwd = self.config.cwd.clone();

        tokio::spawn(async move {
            match cass_core::exec::process_exec_tool_call(
                exec_params,
                sandbox_type,
                &effective_policy,
                sandbox_cwd.as_path(),
    ...
    async fn process_new_conversation(&self, request_id: RequestId, params: NewConversationParams) {
        ...
        let overrides = ConfigOverrides {
            ...
            sandbox_mode,
            model_provider,
            cass_linux_sandbox_exe: self.cass_linux_sandbox_exe.clone(),
            base_instructions,
            developer_instructions,
            compact_prompt,
            include_apply_patch_tool,
    ...
    async fn thread_start(&mut self, request_id: RequestId, params: ThreadStartParams) {
        ...
        let overrides = ConfigOverrides {
            ...
            approval_policy: params
                .approval_policy
                .map(cass_app_server_protocol::AskForApproval::to_core),
            sandbox_mode: params
                .sandbox
                .map(cass_app_server_protocol::SandboxMode::to_core),
        ...
        let path = match params {
            GetConversationSummaryParams::RolloutPath { rollout_path } => {
                if rollout_path.is_relative() {
                    self.config.cass_home.join(&rollout_path)
                } else {
                    rollout_path
                }
            }
            GetConversationSummaryParams::ConversationId { conversation_id } => {
                match cass_core::find_conversation_path_by_id_str(
                    &self.config.cass_home,
                    &conversation_id.to_string(),
                )
                .await
    ...
    async fn handle_resume_conversation(
            ...
            .await
        {
            Ok(NewConversation {
                ...
                let response = cass_app_server_protocol::ResumeConversationResponse {
                    conversation_id,
                    model: session_configured.model.clone(),
                    initial_messages,
                    rollout_path: session_configured.rollout_path.clone(),
    ...
    async fn archive_conversation_common(
        ...
        let rollout_folder = self.config.cass_home.join(cass_core::SESSIONS_SUBDIR);
        ...
        let result: std::io::Result<()> = async {
            let archive_folder = self
                .config
                .cass_home
                .join(cass_core::ARCHIVED_SESSIONS_SUBDIR);
            ...
            loop {
                tokio::select! {
                    ...
                    event = conversation.next_event() => {
                        ...
                        // instead.
                        let method = format!("cass/event/{}", event.msg);
...
async fn apply_bespoke_event_handling(
    event: Event,
    conversation_id: ConversationId,
    conversation: Arc<CassConversation>,
    outgoing: Arc<OutgoingMessageSender>,
    pending_interrupts: Arc<Mutex<HashMap<ConversationId, Vec<RequestId>>>>,
) {
...
fn extract_conversation_summary(
    ...
    let preview = head
        ...
        .find_map(|item| match cass_core::parse_turn_item(&item) {
...
mod tests {
    ...
    async fn read_summary_from_rollout_returns_empty_preview_when_no_user_message() -> Result<()> {
        use cass_protocol::protocol::RolloutItem;
        use cass_protocol::protocol::RolloutLine;
        use cass_protocol::protocol::SessionMetaLine;
        use std::fs;

...
async fn test_send_message_success() -> Result<()> {
    // Spin up a mock completions server that immediately ends the Cass turn.
    // Two Cass turns hit the mock model (session start + send-user-message). Provide two SSE responses.
    let responses = vec![
    ...
    // Create a temporary Cass home with config pointing at the mock server.
...
async fn send_message(
    ...
    let raw_attempt = tokio::time::timeout(
        ...
        mcp.read_stream_until_notification_message("cass/event/raw_response_item"),
    )
    .await;
    assert!(
        raw_attempt.is_err(),
...
async fn test_send_message_session_not_found() -> Result<()> {
    // Start MCP without creating a Cass session
    let cass_home = TempDir::new()?;
    let mut mcp = McpProcess::new(cass_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn read_raw_response_item(
    ...
    .await
    .expect("cass/event/raw_response_item notification timeout")
    .expect("cass/event/raw_response_item notification resp");

    let serde_json::Value::Object(params) = raw_notification
        .params
        .expect("cass/event/raw_response_item should have params")
    else {
        panic!("cass/event/raw_response_item should have params");
    };

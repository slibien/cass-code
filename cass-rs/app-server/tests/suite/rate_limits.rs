...
async fn get_account_rate_limits_requires_auth() -> Result<()> {
    ...
    assert_eq!(
        error.error.message,
        "cass account authentication required to read rate limits"
    );
...
async fn get_account_rate_limits_returns_snapshot() -> Result<()> {
    ...
    write_chatgpt_base_url(cass_home.path(), &server_url)?;
...
fn write_chatgpt_base_url(cass_home: &Path, base_url: &str) -> std::io::Result<()> {

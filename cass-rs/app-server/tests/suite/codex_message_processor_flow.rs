...
use cass_app_server_protocol::InputItem;
...
use cass_app_server_protocol::ServerRequest;
use cass_core::protocol::AskForApproval;
use cass_core::protocol::SandboxPolicy;
use cass_core::protocol_config_types::ReasoningEffort;
use cass_core::protocol_config_types::ReasoningSummary;
use cass_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
async fn test_cass_jsonrpc_conversation_flow() -> Result<()> {
    if env::var(CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Cass sandbox."
        );
        return Ok(());
    }

    let tmp = TempDir::new()?;
    // Temporary Cass home with config pointing at the mock server.
    let cass_home = tmp.path().join("cass_home");
    std::fs::create_dir(&cass_home)?;
    ...
    create_config_toml(&cass_home, &server.uri())?;

    // Start MCP server and initialize.
    let mut mcp = McpProcess::new(&cass_home).await?;
    ...
    let send_user_id = mcp
        .send_send_user_message_request(SendUserMessageParams {
            conversation_id,
            items: vec![cass_app_server_protocol::InputItem::Text {
                text: "text".to_string(),
            }],
        })
        .await?;
    ...
    let task_finished_notification: JSONRPCNotification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("cass/event/task_complete"),
    )
    .await??;
...
async fn test_send_user_turn_changes_approval_policy_behavior() -> Result<()> {
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": cass_core::protocol::ReviewDecision::Approved }),
    )
    .await?;
...
async fn test_send_user_turn_updates_sandbox_and_cwd_between_turns() -> Result<()> {
    ...
    let exec_begin_notification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("cass/event/exec_command_begin"),
    )
    .await??;
...
fn create_config_toml(cass_home: &Path, server_uri: &str) -> std::io::Result<()> {

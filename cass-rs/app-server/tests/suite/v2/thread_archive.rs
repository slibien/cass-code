...
use cass_core::ARCHIVED_SESSIONS_SUBDIR;
...
async fn thread_archive_moves_rollout_into_archived_directory() -> Result<()> {
    ...
    let rollout_path = find_conversation_path_by_id_str(cass_home.path(), &thread.id)
        .await?
    ...
    // Verify file moved.
    let archived_directory = cass_home.path().join(ARCHIVED_SESSIONS_SUBDIR);

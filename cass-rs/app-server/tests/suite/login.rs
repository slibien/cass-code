...
use cass_app_server_protocol::LogoutChatGptResponse;
...
fn create_config_toml(cass_home: &Path) -> std::io::Result<()> {
...
async fn logout_chatgpt_removes_auth() -> Result<()> {
    ...
    create_config_toml(cass_home.path())?;
    login_with_api_key(
        cass_home.path(),
        "sk-test-key",
...
async fn login_and_cancel_chatgpt() -> Result<()> {
    ...
    let maybe_note = timeout(
        ...
        mcp.read_stream_until_notification_message("cass/event/login_chat_gpt_complete"),
    )
    .await;
...
fn create_config_toml_forced_login(cass_home: &Path, forced_method: &str) -> std::io::Result<()> {
    let config_toml = cass_home.join("config.toml");
    let contents = format!(
        r#"
model = "mock-model"
...
async fn login_chatgpt_rejected_when_forced_api() -> Result<()> {
    let cass_home = TempDir::new()?;
    create_config_toml_forced_login(cass_home.path(), "api")?;
...
async fn login_chatgpt_includes_forced_workspace_query_param() -> Result<()> {
    let cass_home = TempDir::new()?;
    create_config_toml_forced_workspace(cass_home.path(), "ws-forced")?;

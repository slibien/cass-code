...
use cass_app_server_protocol::CancelLoginChatGptParams;
use cass_app_server_protocol::ClientInfo;
use cass_app_server_protocol::ClientNotification;
use cass_app_server_protocol::FeedbackUploadParams;
use cass_app_server_protocol::GetAuthStatusParams;
use cass_app_server_protocol::InitializeParams;
...
use cass_app_server_protocol::JSONRPCMessage;
...
impl McpProcess {
    pub async fn new(cass_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(cass_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let std_cmd = StdCommand::cargo_bin("cass-app-server")
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("cass-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        let params = Some(serde_json::to_value(InitializeParams {
            client_info: ClientInfo {
                name: "cass-app-server-tests".to_string(),
                title: None,
    ...
    fn enqueue_user_message(&mut self, notification: JSONRPCNotification) {
        if notification.method == "cass/event/user_message" {

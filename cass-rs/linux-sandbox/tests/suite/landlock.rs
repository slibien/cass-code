#![cfg(target_os = "linux")]
use cass_core::config::types::ShellEnvironmentPolicy;
use cass_core::error::CassErr;
use cass_core::error::SandboxErr;
use cass_core::exec::ExecParams;
use cass_core::exec::SandboxType;
use cass_core::exec::process_exec_tool_call;
...
async fn run_cmd(cmd: &[&str], writable_roots: &[PathBuf], timeout_ms: u64) {
    ...
    let cass_linux_sandbox_exe = Some(PathBuf::from(sandbox_program));
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_codex-linux-sandbox");
    let cass_linux_sandbox_exe: Option<PathBuf> = Some(PathBuf::from(sandbox_program));
    ...
    let output = match result {
        Ok(output) => output,
        Err(CassErr::Sandbox(SandboxErr::Denied { output })) => *output,
        _ => {

...
pub fn set_user_agent_suffix(suffix: &str) {
    if let Ok(mut guard) = cass_core::default_client::USER_AGENT_SUFFIX.lock() {
...
pub async fn build_chatgpt_headers() -> HeaderMap {
    ...
    set_user_agent_suffix("cass_cloud_tasks_tui");
    let ua = cass_core::default_client::get_cass_user_agent();
    let mut headers = HeaderMap::new();
    headers.insert(
        USER_AGENT,
        HeaderValue::from_str(&ua).unwrap_or(HeaderValue::from_static("cass-cli")),
    );
    if let Ok(home) = cass_core::config::find_cass_home() {
        ...
        .unwrap_or_default();
        let am = cass_login::AuthManager::new(home, false, store_mode);

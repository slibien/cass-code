...
struct ApplyJob {
    task_id: cass_cloud_tasks_client::TaskId,
    diff_override: Option<String>,
}

struct BackendContext {
    backend: Arc<dyn cass_cloud_tasks_client::CloudBackend>,
    base_url: String,
}
...
async fn init_backend(user_agent_suffix: &str) -> anyhow::Result<BackendContext> {
    ...
    if use_mock {
        return Ok(BackendContext {
            backend: Arc::new(cass_cloud_tasks_client::MockClient),
            base_url,
        });
    }

    let ua = cass_core::default_client::get_cass_user_agent();
    let mut http = cass_cloud_tasks_client::HttpClient::new(base_url.clone())?.with_user_agent(ua);
    ...
    let auth = match cass_core::config::find_cass_home()
        .ok()
        .map(|home| {
            let store_mode = cass_core::config::Config::load_from_base_config_with_overrides(
                cass_core::config::ConfigToml::default(),
                cass_core::config::ConfigOverrides::default(),
                home.clone(),
            )
            .map(|cfg| cfg.cli_auth_credentials_store_mode)
            .unwrap_or_default();
            cass_login::AuthManager::new(home, false, store_mode)
        })
        .and_then(|am| am.auth())
    {
        Some(auth) => auth,
...
async fn run_exec_command(args: crate::cli::ExecCommand) -> anyhow::Result<()> {
    ...
    let ctx = init_backend("cass_cloud_tasks_exec").await?;
    let prompt = resolve_query_input(query)?;
    let env_id = resolve_environment_id(&ctx, &environment).await?;
    let created = cass_cloud_tasks_client::CloudBackend::create_task(
        &*ctx.backend,
        &env_id,
        &prompt,
        "main",
...
fn level_from_status(status: cass_cloud_tasks_client::ApplyStatus) -> app::ApplyResultLevel {
    match status {
        cass_cloud_tasks_client::ApplyStatus::Success => app::ApplyResultLevel::Success,
        cass_cloud_tasks_client::ApplyStatus::Partial => app::ApplyResultLevel::Partial,
        cass_cloud_tasks_client::ApplyStatus::Error => app::ApplyResultLevel::Error,
    }
}
...
fn spawn_preflight(
    ...
    tokio::spawn(async move {
        ...
        let result = cass_cloud_tasks_client::CloudBackend::apply_task_preflight(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
fn spawn_apply(
    ...
    tokio::spawn(async move {
        ...
        let result = cass_cloud_tasks_client::CloudBackend::apply_task(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
pub async fn run_main(cli: Cli, _cass_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    let BackendContext { backend, .. } = init_backend("cass_cloud_tasks_tui").await?;
    let backend = backend;
    ...
    append_error_log(format!(
        ...
        force_internal,
        cass_core::default_client::get_cass_user_agent()
    ));
            ...
            Some(()) = redraw_rx.recv() => {
                ...
                if let Some(page) = app.new_task.as_mut() {
                    ...
                    if page.composer.is_in_paste_burst() {
                        let _ = frame_tx.send(Instant::now() + cass_tui::ComposerInput::recommended_flush_delay());
                    }
                }
                            ...
                            if let Some(ov) = app.diff_overlay.as_mut() {
                                ...
                                if let (Some(turn_id), true) = (turn_id.clone(), !sibling_turn_ids.is_empty())
                                    && ov.attempts.len() == 1 {
                                        ...
                                        tokio::spawn(async move {
                                            match cass_cloud_tasks_client::CloudBackend::list_sibling_attempts(
                                                &*backend,
                                                task_id.clone(),
                                                turn_id,
                                            )
                        ...
                        app::AppEvent::ApplyFinished { id, result } => {
                            ...
                            match result {
                                Ok(outcome) => {
                                    ...
                                    if matches!(outcome.status, cass_cloud_tasks_client::ApplyStatus::Success) {
                        ...
                        if let Some(page) = app.new_task.as_mut() {
                            ...
                            match key.code {
                                ...
                                _ => {
                                    ...
                                    } else if let cass_tui::ComposerAction::Submitted(text) = page.composer.input(key) {
                                            ...
                                            if let Some(env) = page.env_id.clone() {
                                                ...
                                                tokio::spawn(async move {
                                                    let git_ref = if let Ok(cwd) = std::env::current_dir() {
                                                        if let Some(branch) = cass_core::git_info::default_branch_name(&cwd).await {
                                                            branch
                                                        } else if let Some(branch) = cass_core::git_info::current_branch_name(&cwd).await {
                                                            branch
                                                        } else {
                                                            "main".to_string()
                                                        }
                                                    ...
                                                    let result = cass_cloud_tasks_client::CloudBackend::create_task(&*backend, &env, &text, &git_ref, false, best_of_n).await;
                        ...
                        if app.apply_modal.is_some() {
                            ...
                            match key.code {
                                ...
                                KeyCode::Enter => {
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        ...
                                        let title = task.title.clone();
                                        {
                                            ...
                                            tokio::spawn(async move {
                                                match cass_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, diff_id.clone()).await {
                                                    Ok(Some(diff)) => {
                                                        let _ = tx.send(app::AppEvent::DetailsDiffLoaded { id: diff_id, title: diff_title, diff });
                                                    }
                                                    Ok(None) => {
                                                        match cass_cloud_tasks_client::CloudBackend::get_task_text(&*backend, diff_id.clone()).await {
                                                            Ok(text) => {
                                        ...
                                        // Also fetch conversation text even when diff exists
                                        {
                                            ...
                                            let msg_title = title;
                                            tokio::spawn(async move {
                                                if let Ok(text) = cass_cloud_tasks_client::CloudBackend::get_task_text(&*backend, msg_id.clone()).await {
                                                    let evt = app::AppEvent::DetailsMessagesLoaded {
                                                        id: msg_id,
                                                        title: msg_title,
                                                        messages: text.messages,
                                ...
                                KeyCode::Char('a') => {
                                    ...
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        match cass_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, task.id.clone()).await {
                                            Ok(Some(diff)) => {
...
#[cfg(test)]
mod tests {
    use cass_tui::ComposerAction;
    use cass_tui::ComposerInput;
    use crossterm::event::KeyCode;

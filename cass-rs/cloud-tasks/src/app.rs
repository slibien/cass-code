...
use cass_cloud_tasks_client::CloudBackend;
use cass_cloud_tasks_client::TaskId;
use cass_cloud_tasks_client::TaskSummary;
#[derive(Default)]
pub struct App {
...
pub struct AttemptView {
    ...
    pub status: cass_cloud_tasks_client::AttemptStatus,
...
pub enum AppEvent {
    ...
    DetailsMessagesLoaded {
        ...
        attempt_status: cass_cloud_tasks_client::AttemptStatus,
    },
    DetailsFailed {
        id: TaskId,
        title: String,
    ...
    AttemptsLoaded {
        id: TaskId,
        attempts: Vec<cass_cloud_tasks_client::TurnAttempt>,
    },
    /// Background completion of new task submission
    NewTaskSubmitted(Result<cass_cloud_tasks_client::CreatedTask, String>),
    ...
    ApplyFinished {
        id: TaskId,
        result: std::result::Result<cass_cloud_tasks_client::ApplyOutcome, String>,
    },
}
...
mod tests {
    ...
    impl cass_cloud_tasks_client::CloudBackend for FakeBackend {
        async fn list_tasks(
            &self,
            env: Option<&str>,
        ) -> cass_cloud_tasks_client::Result<Vec<TaskSummary>> {
            ...
            for (i, t) in titles.into_iter().enumerate() {
                out.push(TaskSummary {
                    ...
                    title: t.to_string(),
                    status: cass_cloud_tasks_client::TaskStatus::Ready,
                    updated_at: Utc::now(),
                    environment_id: env.map(str::to_string),
                    environment_label: None,
                    summary: cass_cloud_tasks_client::DiffSummary::default(),
                    is_review: false,
        ...
        async fn get_task_diff(
            &self,
            _id: TaskId,
        ) -> cass_cloud_tasks_client::Result<Option<String>> {
            Err(cass_cloud_tasks_client::CloudTaskError::Unimplemented(
                "not used in test",
            ))
        }
        ...
        async fn get_task_messages(
            &self,
            _id: TaskId,
        ) -> cass_cloud_tasks_client::Result<Vec<String>> {
            Ok(vec![])
        }
        async fn get_task_text(
            &self,
            _id: TaskId,
        ) -> cass_cloud_tasks_client::Result<cass_cloud_tasks_client::TaskText> {
            Ok(cass_cloud_tasks_client::TaskText {
                prompt: Some("Example prompt".to_string()),
                messages: Vec::new(),
                turn_id: Some("fake-turn".to_string()),
                sibling_turn_ids: Vec::new(),
                attempt_placement: Some(0),
                attempt_status: cass_cloud_tasks_client::AttemptStatus::Completed,
            })
        }
        ...
        async fn list_sibling_attempts(
            ...
            _task: TaskId,
            _turn_id: String,
        ) -> cass_cloud_tasks_client::Result<Vec<cass_cloud_tasks_client::TurnAttempt>> {
            Ok(Vec::new())
        }
        ...
        ) -> cass_cloud_tasks_client::Result<cass_cloud_tasks_client::ApplyOutcome> {
            Err(cass_cloud_tasks_client::CloudTaskError::Unimplemented(
                "not used in test",
            ))
        }
        ...
        async fn create_task(
            ...
            _qa_mode: bool,
            _best_of_n: usize,
        ) -> cass_cloud_tasks_client::Result<cass_cloud_tasks_client::CreatedTask> {
            Err(cass_cloud_tasks_client::CloudTaskError::Unimplemented(
                "not used in test",
            ))
        }

...
use base64::Engine;
use cass_common::format_env_display::format_env_display;
use cass_core::config::Config;
use cass_core::config::types::McpServerTransportConfig;
use cass_core::config::types::ReasoningSummaryFormat;
use cass_core::protocol::FileChange;
use cass_core::protocol::McpAuthStatus;
...
impl HistoryCell for UpdateAvailableHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let update_instruction = if let Some(update_action) = self.update_action {
            ...
            line![
                "See ",
                "https://github.com/slibien/cass-code".cyan().underlined(),
        ...
        let content = text![
            ...
            "https://github.com/slibien/cass-code/releases/latest"
                .cyan()
                .underlined(),
        ];
...
pub fn new_approval_decision_cell(
    command: Vec<String>,
    decision: cass_core::protocol::ReviewDecision,
) -> Box<dyn HistoryCell> {
    ...
    if is_first_event {
        ...
        let help_lines: Vec<Line<'static>> = vec![
            ...
            Line::from(vec![
                "  ".into(),
                "/init".into(),
                " - create an AGENTS.md file with instructions for Cass".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/status".into(),
            ...
            Line::from(vec![
                "  ".into(),
                "/approvals".into(),
                " - choose what Cass can do without approval".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/model".into(),
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        // Title line rendered inside the box: ">_ OpenAI Cass (vX)"
        let title_spans: Vec<Span<'static>> = vec![
            Span::from(">_ ").dim(),
            Span::from("OpenAI Cass").bold(),
            Span::from(" ").dim(),
...
pub(crate) fn empty_mcp_output() -> PlainHistoryCell {
    let lines: Vec<Line<'static>> = vec![
        ...
        Line::from(vec![
            "    See the ".into(),
            "\u{1b}]8;;https://github.com/slibien/cass-code/blob/main/docs/config.md#mcp_servers\u{7}MCP docs\u{1b}]8;;\u{7}".underlined(),
...
mod tests {
    ...
    use cass_core::config::types::McpServerConfig;
    use cass_core::config::types::McpServerTransportConfig;
    use cass_core::protocol::McpAuthStatus;

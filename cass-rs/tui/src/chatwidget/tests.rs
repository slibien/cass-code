...
use cass_core::protocol::PatchApplyEndEvent;
use cass_core::protocol::RateLimitWindow;
use cass_core::protocol::ReviewCodeLocation;
use cass_core::protocol::ReviewFinding;
use cass_core::protocol::ReviewLineRange;
use cass_core::protocol::ReviewOutputEvent;
use cass_core::protocol::ReviewRequest;
use cass_core::protocol::StreamErrorEvent;
use cass_core::protocol::TaskCompleteEvent;
use cass_core::protocol::TaskStartedEvent;
...
use cass_protocol::plan_tool::PlanItemArg;
use cass_protocol::plan_tool::StepStatus;
...
fn resumed_initial_messages_render_history() {
    ...
    let configured = cass_core::protocol::SessionConfiguredEvent {
...
async fn helpers_are_available_and_do_not_panic() {
    ...
    let cfg = test_config();
    let conversation_manager = Arc::new(ConversationManager::with_auth(CassAuth::from_api_key(
        "test",
    )));
    let auth_manager = AuthManager::from_auth_for_testing(CassAuth::from_api_key("test"));
    let init = ChatWidgetInit {
        ...
        auth_manager,
        feedback: cass_feedback::CassFeedback::new(),
    };
...
fn make_chatwidget_manual() -> (
    ...
    let bottom = BottomPane::new(BottomPaneParams {
        ...
        enhanced_keys_supported: false,
        placeholder_text: "Ask Cass to do anything".to_string(),
...
fn begin_exec(chat: &mut ChatWidget, call_id: &str, raw_cmd: &str) {
    ...
    let parsed_cmd: Vec<ParsedCommand> = cass_core::parse_command::parse_command(&command);
    chat.handle_cass_event(Event {
...
fn slash_init_skips_when_project_doc_exists() {
    ...
    match op_rx.try_recv() {
        ...
        other => panic!("expected no Cass op to be sent, got {other:?}"),
    }
...
fn slash_undo_sends_op() {
    ...
    match rx.try_recv() {
        Ok(AppEvent::CassOp(Op::Undo)) => {}
        other => panic!("expected AppEvent::CassOp(Op::Undo), got {other:?}"),
    }
}
...
fn review_commit_picker_shows_subjects_without_timestamps() {
    ...
    let entries = vec![
        cass_core::git_info::CommitLogEntry {
...
fn custom_prompt_submit_sends_review_op() {
    ...
    // Expect AppEvent::CassOp(Op::Review { .. }) with trimmed prompt
    let evt = rx.try_recv().expect("expected one app event");
    match evt {
        AppEvent::CassOp(Op::Review { review_request }) => {
            assert_eq!(
...
fn custom_prompt_enter_empty_does_not_send() {
    ...
    // No AppEvent::CassOp should be sent
...
fn interrupt_exec_marks_failed_snapshot() {
    ...
    chat.handle_cass_event(Event {
        id: "call-int".into(),
        msg: EventMsg::TurnAborted(cass_core::protocol::TurnAbortedEvent {
...
fn approvals_popup_includes_wsl_note_for_auto_mode() {
    ...
    assert_eq!(
        popup.contains("Cass forced your settings back to Read Only on this Windows machine."),
                ...
                if let Some(payload) = v.get("payload") {
                    ...
                    let ev = match ev {
                        Event {
                            ...
                            let parsed_cmd = cass_core::parse_command::parse_command(&e.command);
                            Event {
                                id: ev.id,
                                msg: EventMsg::ExecCommandBegin(ExecCommandBeginEvent {
...
//
// Synthesizes a Cass ExecApprovalRequest event to trigger the approval modal
...
fn apply_patch_approval_sends_op_with_submission_id() {
    ...
    // Expect a CassOp with PatchApproval carrying the submission id, not call id
    let mut found = false;
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::CassOp(Op::PatchApproval { id, decision }) = app_ev {
            assert_eq!(id, "sub-123");
            assert_matches!(decision, cass_core::protocol::ReviewDecision::Approved);
            found = true;
            break;
        }
    }
...
fn apply_patch_full_flow_integration_like() {
    ...
    // 2) User approves via 'y' and App receives a CassOp
    chat.handle_key_event(KeyEvent::new(KeyCode::Char('y'), KeyModifiers::NONE));
    let mut maybe_op: Option<Op> = None;
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::CassOp(op) = app_ev {
            maybe_op = Some(op);
            break;
        }
    }
    let op = maybe_op.expect("expected CassOp after key press");

    // 3) App forwards to widget.submit_op, which pushes onto cass_op_tx
    chat.submit_op(op);
    let forwarded = op_rx
        .try_recv()

...
use cass_core::config::types::Notifications;
use cass_core::git_info::current_branch_name;
use cass_core::git_info::local_git_branches;
use cass_core::project_doc::DEFAULT_PROJECT_DOC_FILENAME;
use cass_core::protocol::AgentMessageDeltaEvent;
use cass_core::protocol::AgentMessageEvent;
use cass_core::protocol::AgentReasoningDeltaEvent;
use cass_core::protocol::AgentReasoningEvent;
use cass_core::protocol::AgentReasoningRawContentDeltaEvent;
use cass_core::protocol::AgentReasoningRawContentEvent;
use cass_core::protocol::ApplyPatchApprovalRequestEvent;
use cass_core::protocol::BackgroundEventEvent;
use cass_core::protocol::DeprecationNoticeEvent;
use cass_core::protocol::ErrorEvent;
use cass_core::protocol::Event;
use cass_core::protocol::EventMsg;
use cass_core::protocol::ExecApprovalRequestEvent;
use cass_core::protocol::ExecCommandBeginEvent;
use cass_core::protocol::ExecCommandEndEvent;
use cass_core::protocol::ExitedReviewModeEvent;
use cass_core::protocol::ListCustomPromptsResponseEvent;
use cass_core::protocol::McpListToolsResponseEvent;
use cass_core::protocol::McpToolCallBeginEvent;
use cass_core::protocol::McpToolCallEndEvent;
use cass_core::protocol::Op;
use cass_core::protocol::PatchApplyBeginEvent;
use cass_core::protocol::RateLimitSnapshot;
use cass_core::protocol::ReviewRequest;
use cass_core::protocol::StreamErrorEvent;
use cass_core::protocol::TaskCompleteEvent;
use cass_core::protocol::TokenUsage;
use cass_core::protocol::TokenUsageInfo;
use cass_core::protocol::TurnAbortReason;
use cass_core::protocol::TurnDiffEvent;
use cass_core::protocol::UndoCompletedEvent;
use cass_core::protocol::UndoStartedEvent;
use cass_core::protocol::UserMessageEvent;
use cass_core::protocol::ViewImageToolCallEvent;
use cass_core::protocol::WarningEvent;
use cass_core::protocol::WebSearchBeginEvent;
use cass_core::protocol::WebSearchEndEvent;
use cass_protocol::ConversationId;
use cass_protocol::parse_command::ParsedCommand;
use cass_protocol::user_input::UserInput;
...
use chrono::Local;
use cass_common::approval_presets::ApprovalPreset;
use cass_common::approval_presets::builtin_approval_presets;
use cass_common::model_presets::ModelPreset;
use cass_common::model_presets::builtin_model_presets;
use cass_core::AuthManager;
use cass_core::ConversationManager;
use cass_core::protocol::AskForApproval;
use cass_core::protocol::SandboxPolicy;
use cass_core::protocol_config_types::ReasoningEffort as ReasoningEffortConfig;
use cass_file_search::FileMatch;
use cass_protocol::plan_tool::UpdatePlanArgs;
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) feedback: cass_feedback::CassFeedback,
}
...
pub(crate) struct ChatWidget {
    ...
    feedback: cass_feedback::CassFeedback,
...
impl ChatWidget {
    ...
    fn on_session_configured(&mut self, event: cass_core::protocol::SessionConfiguredEvent) {
        self.bottom_pane
    ...
    fn on_exec_command_output_delta(
        &mut self,
        _ev: cass_core::protocol::ExecCommandOutputDeltaEvent,
    ) {
    ...
    fn on_patch_apply_end(&mut self, event: cass_core::protocol::PatchApplyEndEvent) {
        let ev2 = event.clone();
        self.defer_or_handle(
    ...
    fn on_get_history_entry_response(
        &mut self,
        event: cass_core::protocol::GetHistoryEntryResponseEvent,
    ) {
        let cass_core::protocol::GetHistoryEntryResponseEvent {
            offset,
            log_id,
            entry,
        } = event;
    ...
    pub(crate) fn handle_patch_apply_end_now(
        &mut self,
        event: cass_core::protocol::PatchApplyEndEvent,
    ) {
    ...
    pub(crate) fn new(
        ...
        let cass_op_tx = spawn_agent(config.clone(), app_event_tx.clone(), conversation_manager);
    ...
    pub(crate) fn new_from_existing(
        common: ChatWidgetInit,
        conversation: std::sync::Arc<cass_core::CassConversation>,
        session_configured: cass_core::protocol::SessionConfiguredEvent,
    ) -> Self {
        let ChatWidgetInit {
            config,
            frame_requester,
    ...
    fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::Compact => {
                ...
                self.app_event_tx.send(AppEvent::CassOp(Op::Compact));
            }
            SlashCommand::Review => {
            ...
            SlashCommand::Logout => {
                if let Err(e) = cass_core::auth::logout(
            ...
            SlashCommand::Undo => {
                self.app_event_tx.send(AppEvent::CassOp(Op::Undo));
            }
            SlashCommand::Diff => {
            ...
            SlashCommand::TestApproval => {
                ...
                use cass_core::protocol::ApplyPatchApprovalRequestEvent;
                use cass_core::protocol::FileChange;

                self.app_event_tx.send(AppEvent::CassEvent(Event {
                    id: "1".to_string(),
    ...
    pub(crate) fn handle_cass_event(&mut self, event: Event) {
    ...
    fn dispatch_event_msg(&mut self, id: Option<String>, msg: EventMsg, from_replay: bool) {
        match msg {
            ...
            _ => {
                tracing::trace!("handle_cass_event: {:?}", msg);
            }
        }
    ...
    fn on_exited_review_mode(&mut self, review: ExitedReviewModeEvent) {
        ...
        if let Some(output) = review.review_output {
            ...
            if output.findings.is_empty() {
                ...
                let message_text =
                    cass_core::review_format::format_review_findings_block(&output.findings, None);
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::CassOp(Op::OverrideTurnContext {
                cwd: None,
    ...
    pub(crate) fn open_model_popup(&mut self) {
        ...
        self.bottom_pane.show_selection_view(SelectionViewParams {
            ...
            subtitle: Some("Switch the model for this and future Cass CLI sessions".to_string()),
    ...
    fn apply_model_and_effort(&self, model: String, effort: Option<ReasoningEffortConfig>) {
        self.app_event_tx
            .send(AppEvent::CassOp(Op::OverrideTurnContext {
                cwd: None,
                approval_policy: None,
                sandbox_policy: None,
    ...
    pub(crate) fn open_approvals_popup(&mut self) {
            ...
            .forced_auto_mode_downgraded_on_windows
        {
            ...
            let mut header = ColumnRenderable::new();
            header.push(line![
                "Cass forced your settings back to Read Only on this Windows machine.".bold()
            ]);
            header.push(line![
                "To re-enable Auto mode, run Cass inside Windows Subsystem for Linux (WSL) or enable Full Access manually.".dim()
                ]);
            Box::new(header)
        } else {
            Box::new(())
        ...
        for preset in presets.into_iter() {
            ...
            let description = if cfg!(target_os = "windows")
                && preset.id == "auto"
                && cass_core::get_platform_sandbox().is_none()
            {
                Some(format!(
    ...
    pub(crate) fn open_full_access_confirmation(&mut self, preset: ApprovalPreset) {
        ...
        let info_line = Line::from(vec![
            "When Cass runs with full access, it can edit any file on your computer and run commands with network, without your approval. "
                .into(),
    ...
    pub(crate) fn open_windows_auto_mode_instructions(&mut self) {
        ...
        header.push(line!["Run Cass inside WSL to enable sandboxed commands."]);
        header.push(line![""]);
    ...
    pub(crate) fn submit_op(&self, op: Op) {
        ...
        if let Err(e) = self.cass_op_tx.send(op) {
        ...
        items.push(SelectionItem {
            ...
            actions: vec![Box::new(
                move |tx: &AppEventSender| {
                    tx.send(AppEvent::CassOp(Op::Review {
        ...
        for option in branches {
            ...
            items.push(SelectionItem {
                ...
                actions: vec![Box::new(move |tx3: &AppEventSender| {
                    tx3.send(AppEvent::CassOp(Op::Review {
                        review_request: ReviewRequest {
                            prompt: format!(
    ...
    pub(crate) async fn show_review_commit_picker(&mut self, cwd: &Path) {
        let commits = cass_core::git_info::recent_commits(cwd, 100).await;
...
pub(crate) fn show_review_commit_picker_with_entries(
    chat: &mut ChatWidget,
    entries: Vec<cass_core::git_info::CommitLogEntry>,
) {

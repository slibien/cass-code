...
use cass_ansi_escape::ansi_escape_line;
...
impl App {
    ...
    async fn handle_event(&mut self, tui: &mut tui::Tui, event: AppEvent) -> Result<bool> {
        match event {
            ...
            AppEvent::CassEvent(event) => {
                self.chat_widget.handle_cass_event(event);
            }
            ...
            AppEvent::CassOp(op) => self.chat_widget.submit_op(op),
            ...
            AppEvent::PersistFullAccessWarningAcknowledged => {
                if let Err(err) = ConfigEditsBuilder::new(&self.config.cass_home)
    ...
    pub(crate) fn token_usage(&self) -> cass_core::protocol::TokenUsage {
...
mod tests {
    ...
    fn make_test_app() -> App {
        ...
        let server = Arc::new(ConversationManager::with_auth(CassAuth::from_api_key(
            "Test API Key",
        )));
        let auth_manager =
            AuthManager::from_auth_for_testing(CassAuth::from_api_key("Test API Key"));
        let file_search = FileSearchManager::new(config.cwd.clone(), app_event_tx.clone());

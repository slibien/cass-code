...
pub async fn run_main(
    ...
    let overrides_cli = cass_common::CliConfigOverrides { raw_overrides };
    ...
    let log_dir = cass_core::config::log_dir(&config)?;
    ...
    // Cass CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("cass-tui.log"))?;
    ...
    let env_filter = || {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("cass_core=info,cass_tui=info,cass_rmcp_client=info")
        })
    };
    ...
    let feedback = cass_feedback::CassFeedback::new();
...
async fn run_ratatui_app(
            ...
            match update_prompt::run_update_prompt_if_needed(&mut tui, &initial_config).await? {
                ...
                UpdatePromptOutcome::RunUpdate(action) => {
                    ...
                    return Ok(AppExitInfo {
                        token_usage: cass_core::protocol::TokenUsage::default(),
    ...
    let auth_manager = AuthManager::shared(
        initial_config.cass_home.clone(),
        false,
    ...
    let resume_selection = if let Some(id_str) = cli.resume_session_id.as_deref() {
        match find_conversation_path_by_id_str(&config.cass_home, id_str).await? {
...
fn get_login_status(config: &Config) -> LoginStatus {
    if config.model_provider.requires_openai_auth {
        ...
        let cass_home = config.cass_home.clone();
        match CassAuth::from_auth_storage(&cass_home, config.cli_auth_credentials_store_mode) {
...
mod tests {
    ...
    use cass_core::config::ProjectConfig;
    use cass_core::set_windows_sandbox_enabled;

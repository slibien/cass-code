...
use cass_core::config::set_project_trusted;
use cass_core::git_info::resolve_root_git_project_for_trust;
...
impl WidgetRef for &TrustDirectoryWidget {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        column.push(Line::from(vec![
            "> ".into(),
            "You are running Cass in ".bold(),
            self.cwd.to_string_lossy().to_string().into(),
        ]));
        column.push("");

        let guidance = if self.is_git_repo {
            "Since this folder is version controlled, you may wish to allow Cass to work in this folder without asking for approval."
        } else {
        ...
        if self.is_git_repo {
            options.push((
                "Yes, allow Cass to work in this folder without asking for approval",
            ...
            options.push((
                "Allow Cass to work in this folder without asking for approval",
                TrustDirectorySelection::Trust,
            ));
            options.push((
...
impl TrustDirectoryWidget {
    fn handle_trust(&mut self) {
        ...
        if let Err(e) = set_project_trusted(&self.cass_home, &target) {

...
pub(crate) const WSL_INSTRUCTIONS: &str = r#"Install WSL2 by opening PowerShell as Administrator and running:
    ...
    # Install and run Cass in WSL
    npm install --global @openai/codex
    codex

    # Additional details and instructions for how to install and run Cass in WSL:
    https://developers.orion-ai.dev/cass-code/windows"#;
...
impl WindowsSetupWidget {
    pub fn new(cass_home: PathBuf) -> Self {
        Self {
            cass_home,
            selection: None,
    ...
    fn handle_continue(&mut self) {
        ...
        match ConfigEditsBuilder::new(&self.cass_home)
...
impl WidgetRef for &WindowsSetupWidget {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        let mut lines: Vec<Line> = vec![
            Line::from(vec![
                "> ".into(),
                "To use all Cass features, we recommend running Cass in Windows Subsystem for Linux (WSL2)".bold(),
            ]),
            Line::from(vec!["  ".into(), "WSL allows Cass to run Agent mode in a sandboxed environment with better data protections in place.".into()]),
            Line::from(vec!["  ".into(), "Learn more: https://developers.orion-ai.dev/cass-code/windows".into()]),
            Line::from(""),
        ];

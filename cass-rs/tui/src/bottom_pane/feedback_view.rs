...
const BASE_ISSUE_URL: &str = "https://github.com/slibien/cass-code/issues/new?template=2-bug-report.yml";
...
pub(crate) struct FeedbackNoteView {
    ...
    snapshot: cass_feedback::CassLogSnapshot,
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::render::renderable::Renderable>> = vec![
        ...
        Line::from(vec!["  • ".into(), "cass-logs.log".into()]).into(),
    ];
    ...
    super::SelectionViewParams {
        ...
        items: vec![
            super::SelectionItem {
                name: "Yes".to_string(),
                description: Some(
                    "Share the current Cass session logs with the team for troubleshooting."
                        .to_string(),
                ),
...
mod tests {
    ...
    fn make_view(category: FeedbackCategory) -> FeedbackNoteView {
        ...
        let snapshot = cass_feedback::CassFeedback::new().snapshot(None);

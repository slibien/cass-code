...
use cass_core::protocol::SandboxRiskLevel;
...
impl ApprovalOverlay {
    ...
    fn handle_exec_decision(&self, id: &str, command: &[String], decision: ReviewDecision) {
        ...
        self.app_event_tx.send(AppEvent::CassOp(Op::ExecApproval {
            id: id.to_string(),
            decision,
        }));
    }
    ...
    fn handle_patch_decision(&self, id: &str, decision: ReviewDecision) {
        self.app_event_tx.send(AppEvent::CassOp(Op::PatchApproval {
            id: id.to_string(),
            decision,
        }));
    }
...
fn exec_options() -> Vec<ApprovalOption> {
    vec![
        ...
        ApprovalOption {
            label: "No, and tell Cass what to do differently".to_string(),
...
mod tests {
    ...
    fn shortcut_triggers_selection() {
        ...
        // We expect at least one CassOp message in the queue.
        let mut saw_op = false;
        while let Ok(ev) = rx.try_recv() {
            if matches!(ev, AppEvent::CassOp(_)) {
                saw_op = true;
                break;
            }
        }
    ...
    fn enter_sets_last_selected_index_without_dismissing() {
        ...
        while let Ok(ev) = rx.try_recv() {
            if let AppEvent::CassOp(Op::ExecApproval { decision: d, .. }) = ev {
                decision = Some(d);
                break;
            }
        }

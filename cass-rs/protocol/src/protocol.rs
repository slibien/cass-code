//! Defines the protocol for a Cass session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Cass:";
...
pub enum Op {
    ...
    /// for a turn of a [`crate::cass_conversation::CassConversation`].
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,
    ...
    /// Request Cass to undo a turn (turn are stacked so it is the same effect as CMD + Z).
    Undo,
...
/// running the command proposed by Cass.
#[derive(
    Debug,
    Clone,
    Copy,

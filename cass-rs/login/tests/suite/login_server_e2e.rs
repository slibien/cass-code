...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = cass_home.clone();

    let opts = ServerOptions {
        cass_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: cass_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = cass_home.join("auth.json");
...
#[tokio::test]
async fn creates_missing_cass_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let cass_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_cass_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        cass_home: first_cass_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: cass_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
        port: 0,
    ...
    let second_cass_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        cass_home: second_cass_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: cass_login::CLIENT_ID.to_string(),
        issuer,
        port: login_port,

    ...
    if !resp.status().is_success() {
        ...
        if status == StatusCode::NOT_FOUND {
            return Err(std::io::Error::other(
                "device code login is not enabled for this Cass server. Use the browser login or verify the server URL.",
            ));
        }
...
pub async fn run_device_code_login(opts: ServerOptions) -> std::io::Result<()> {
    ...
    println!(
        "To authenticate:\n  1. Open in your browser: {ANSI_BOLD}https://auth.orion-ai.dev/cass-code/device{ANSI_RESET}\n  2. Enter the one-time code below within 15 minutes:\n\n     {ANSI_BOLD}{}{ANSI_RESET}\n",
        uc.user_code
    );

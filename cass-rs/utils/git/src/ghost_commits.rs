...
const DEFAULT_COMMIT_MESSAGE: &str = "cass snapshot";
...
pub fn create_ghost_commit(
    ...
    let index_tempdir = Builder::new().prefix("cass-git-index-").tempdir()?;
...
mod tests {
    ...
    fn restore_from_subdirectory_restores_files_relatively() -> Result<(), GitToolingError> {
        ...
        assert!(!workspace.join("cass-rs").exists());
    ...
    fn restore_from_subdirectory_preserves_parent_vscode() -> Result<(), GitToolingError> {
        ...
        let workspace = repo.join("cass-rs");
